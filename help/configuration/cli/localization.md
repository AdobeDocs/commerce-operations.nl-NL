...
|--- |--- |--- |
| `<path to directory to translate>` | Pad naar een map met vertaalbare code; met andere woorden, PHP-, PHTML- of XML-bestanden met zinnen die moeten worden vertaald.<br><br> het hulpmiddel begint bij de weg te zoeken die u ingaat en alle dossiers en subdirectories zoekt het bevat.<br><br> gebruik deze parameter niet als u `-m --magento` gebruikt. | Ja (woordenboeken), nee (pakketten). |
| `-m --magento` | Vereist voor het maken van een taalpakket op basis van dit vertaalwoordenboek. Indien gebruikt, zoekt naar de folders die bak/magento bevatten. Deze optie voegt thema&#39;s of modules aan elke lijn in het woordenboek toe.<br><br> A volgt steekproef:<br><br> &quot;Geen Gevonden Punten&quot;, &quot;Geen Gevonden Punten&quot;, module, Magento_Wishlist | Nee |
...
Commerce gebruikt de plaatsaanduidingen om contextwaarden in te voegen; zij worden _niet_ gebruikt voor vertalingen. Bijvoorbeeld:
...
| `-m --mode` | Als er een doelbestand bestaat, geeft u op of het bestaande taalpakket moet worden vervangen of moet worden samengevoegd met het nieuwe taalpakket. Het samenvoegen treedt om het even welke uitdrukkingen met voeten die bestonden en nieuwe degenen toevoegen.<br><br> Waarden: samenvoegen of vervangen (standaard). | Nee |
