...
- [Wijzigingen in de beheerder](#the-commerce-admin)
- [Uitsnede installeren en verwijderen](#install-and-remove-cron)

Dit onderwerp bespreekt ook het [&#x200B; geadviseerde werkschema &#x200B;](#recommended-pipeline-deployment-workflow) voor pijpleidingsplaatsing en verstrekt sommige voorbeelden om u te helpen begrijpen hoe het werkt.
...
Gebruik de opdracht [`magento config:set` of `magento config:set --lock`](../cli/set-configuration-values.md#set-values) om de instellingen in de `config.php` - of `env.php` -bestanden te wijzigen.
