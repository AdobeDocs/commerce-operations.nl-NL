...
description: Leer hoe u Adobe Commerce-modules vanaf de opdrachtregel inschakelt, uitschakelt en de status ervan controleert met de status van de module en verwante opties.
...
Met Adobe Commerce kunt u de momenteel beschikbare modules in- of uitschakelen. met andere woorden, een module die door Adobe wordt geleverd of een module van een derde die momenteel beschikbaar is.
...
* Afhankelijkheden worden voor elke module gedeclareerd in het veld `require` in het Adobe Commerce `composer.json` -bestand. Conflicten worden gedeclareerd in het veld `conflict` in de `composer.json` -bestanden van modules. Wij gebruiken die informatie om een gebiedsdeelgrafiek te bouwen: `A->B` betekent module A afhankelijk is van module B.
