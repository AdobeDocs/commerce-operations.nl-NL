...
* Productlijsten worden nu correct weergegeven in de Admin. Eerder werd de productlijst niet weergegeven en deze fout werd weergegeven door de Magento Open Source:  `Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "<ID>" already exists`. [GitHub-33145](https://github.com/magento/magento2/issues/33145)
...
* U kunt nu de productlimiet per pagina wijzigen die in een categorie wordt weergegeven wanneer **Categoriepaginering onthouden**  is ingeschakeld. Eerder was de cookie `form_key` en UI `form_key` afwijkt en deze fout wordt weergegeven in de Magento Open Source:  `Invalid Form Key. Please refresh the page`.
...
* Magento Open Source geeft nu configureerbare kenmerken weer zoals wordt verwacht tijdens het maken van een globaal `select` kenmerken via een patchscript. Eerder waren in aanmerking komende algemene kenmerken verborgen.
...
* Magento Open Source geeft nu een foutbericht weer op de pagina Verzending wanneer een winkelier een ongeldige e-mailindeling invoert nadat de winkelier erop klikt **Opdracht plaatsen**. Eerder werd het foutbericht weergegeven op de pagina Betaling. [GitHub-33590](https://github.com/magento/magento2/issues/33590
...
* Magento Open Source leidt niet langer tot een `trim(): Passing null to parameter #1 ($string) of type string is deprecated` fout wanneer de verbinding AMPQ zonder SSL configuratie wordt gevormd.
...
* De **Winkel** >  **Configuratie** > **Services** de pagina toont nu de informatie van het Web API van de Magento Open Source zoals verwacht wanneer de Toegang van het Middel aan wordt geplaatst  **Aangepast** op het tabblad Rolresources. [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* De vragen van de Elasticsearch werken nu zoals verwacht wanneer `int` is geconfigureerd als doorzoekbare back-end `type` kenmerk. Eerder heeft de applicationOpen Source een `Elasticsearch\Common\Exceptions\BadRequest400Exception` uitzondering.
...
* De Magento Open Source toont nu een nauwkeurige suggestie van onderzoeksresultaten tellend op de storefront in plaatsingen waar de Suggesties van het Onderzoek en de **Aantal resultaten tonen voor elke suggestie** instellen is ingeschakeld. Eerder was het aantal dat naast de trefwoorden werd weergegeven nul.
...
* Correctie van PHP-fouten op de `catalogsearch/advanced/result` en  `catalogsearch/advanced/index` pagina&#39;s.  Eerder werd deze fout door de Magento Open Source weergegeven wanneer een array werd doorgegeven in een geavanceerd zoektekenreeks:  `Warning: trim() expects parameter 1 to be string, array given | magento/module-catalog-search`. [GitHub-33586](https://github.com/magento/magento2/issues/33586)
