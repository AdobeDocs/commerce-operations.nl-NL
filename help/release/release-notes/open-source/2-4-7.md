---
title: Magento Open Source 2.4.7 release opmerkingen
...
Hoewel de code voor deze functies wordt gebundeld met releases van de Magento Open Source-kerncode, worden verschillende van deze projecten ook onafhankelijk van elkaar vrijgegeven. Bugfixes voor deze projecten worden gedocumenteerd in de afzonderlijke, projectspecifieke release-informatie die beschikbaar is in de documentatie voor elk project.
...
Deze release bevat dezelfde beveiligingsfixes en platformbeveiligingsverbeteringen die zijn opgenomen in Adobe Commerce 2.4.6-p5, 2.4.5-p7 en 2.4.4-p8. Zie [het Adobe-beveiligingsbulletin](https://helpx.adobe.com/nl/security/products/magento/apsb24-18.html) voor de meest recente bespreking van deze opgeloste problemen.
...
* **Optimalisatie van het standaard proces voor het genereren** van beheerders-URL&#39;s. Het genereren van de standaard beheerders-URL is geoptimaliseerd voor meer willekeur, waardoor gegenereerde URL&#39;s minder voorspelbaar zijn. <!-- AC-9028 -->

* Een nieuwe configuratie-instelling voor de cache op volledige pagina&#39;s kan helpen om de risico&#39;s van het HTTP-eindpunt `{BASE-URL}/page_cache/block/esi` te beperken. Dit eindpunt ondersteunt onbeperkte, dynamisch geladen inhoudsfragmenten van Commerce-lay-outgrepen en blokstructuren. De nieuwe **[!UICONTROL Handles params size]** configuratie-instelling stelt de waarde in van de parameter van dit eindpunt, die het maximaal toegestane aantal ingangen `handles` per API bepaalt. De standaardwaarde van deze eigenschap is 100. Handelaars kunnen deze waarde wijzigen via Beheer (**[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > **[!UICONTROL System]** > **[!UICONTROL Full Page Cache]** > **[!UICONTROL Handles params size]** ). Zie [ de toepassing van Commerce vormen om Vierkant ](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/cache/configure-varnish-commerce.html?lang=nl-NL) te gebruiken. <!-- AC-9113 -->

* **toegevoegde Steun van de Integriteit Subresource (SRI)** om aan PCI 4.0 vereisten voor controle van manuscriptintegriteit op betalingspagina&#39;s te voldoen. De ondersteuning van Subresource Integrity (SRI) biedt integriteitshashes voor alle JavaScript-elementen die zich in het lokale bestandssysteem bevinden. De standaard SRI-functie wordt alleen geïmplementeerd op de betalingspagina&#39;s voor de Admin- en storefront-gebieden. Winkeliers kunnen de standaardconfiguratie echter uitbreiden naar andere pagina&#39;s. Zie [Integriteit](https://developer.adobe.com/commerce/php/development/security/subresource-integrity/) van subresources in de _Commerce PHP Developer Guide_.<!--AC-11513-->
...
* **Native tarief beperkt voor betalingsinformatie die door REST en GraphQL APIs wordt overgebracht**. De handelaren kunnen tarief nu vormen die [&#128279;](https://experienceleague.adobe.com/nl/docs/commerce-admin/config/sales/sales#rate-limiting) voor de betalingsinformatie beperken die REST en GraphQL wordt overgebracht.  Deze extra beschermingslaag ondersteunt het voorkomen van kaartaanvallen en vermindert mogelijk het aantal kaartaanvallen waarbij veel creditcardnummers tegelijk worden getest. Dit is een verandering in het standaardgedrag van een bestaand REST eindpunt. Zie [Tariefbeperking](https://developer.adobe.com/commerce/webapi/get-started/rate-limiting/).

* Het standaardgedrag van de [isEmailAvailable](https://developer.adobe.com/commerce/webapi/graphql/schema/customer/queries/is-email-available/) GraphQL-query en het ([V1/customers/isEmailAvailable) REST-eindpunt](https://adobe-commerce.redoc.ly/2.4.7-admin/tag/customersisEmailAvailable/#operation/PostV1CustomersIsEmailAvailable) is gewijzigd. Standaard retourneren `true`de API&#39;s nu altijd . Winkeliers kunnen het oorspronkelijke gedrag inschakelen door de *[optie Inloggen](https://experienceleague.adobe.com/nl/docs/commerce-admin/config/sales/checkout)* voor gastafhandeling inschakelen in de beheerder in te stellen op `yes`, maar als u dit doet, kunnen klantgegevens worden blootgesteld aan niet-geverifieerde gebruikers.
...
Alle JavaScript-bibliotheken en NPM-afhankelijkheden in Magento Open Source kerncode zijn geüpdatet naar de nieuwste beschikbare versies. Alle Laminas library dependencies zijn geüpdatet naar de nieuwste versie die compatibel zijn met PHP 8.3. <!-- AC-9667 9665 -->
...
* Toegevoegde ondersteuning voor de nieuwe verzendmethode voor het grondvoordeel van USPS. Dit is een out-of-box integratie met de nieuwe verzendmethode van USPS, USPS Ground Advantage, die in juli 2023 is uitgebracht. Deze nieuwe integratie kan worden gebruikt om de verzendkosten op te halen en leveringen te plannen en terug te keren via de verzendservice van USPS. De USPS Ground Advantage-verzendmethode vervangt deze verzendmethoden, die buiten gebruik werden gesteld toen de USPS Ground Advantage-verzendmethode werd uitgebracht:
   ...
   * Perceel Selecteer Grond <!-- AC-9182 -->
...
* **3DS-ondersteuning voor[!DNL Google Pay]**—Inclusief 3DS-verificatieondersteuning voor de [!DNL Google Pay] niet-tokenized kaarten. Zie de [Braintree-documentatie](https://developer.paypal.com/braintree/docs/guides/3d-secure/client-side/android/v3#using-3d-secure-with-google-pay) voor meer informatie.
...
* **Verbeteringen in het maken van winkelwagenmutaties**. De `createGuestCart` -mutatie is toegevoegd ter vervanging van de vervangen `createEmptyCart` -mutatie. Eerder, als u `createEmptyCart` gebruikte, kon u niet bepalen of de kar voor een gast of het programma geopende klant was. <!-- LYNX-224 -->
...
* **Steun voor orderannulering**. De [annuleerOrder-mutatie](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/) stelt een klant in staat om een bestelling te annuleren door zijn ID en een annuleringsreden door te geven. <!-- LYNX-197 -->

   * De velden nieuw `order_cancellation_enabled` en `order_cancellation_reasons.description` antwoord in de `storeConfig`
Query Support Door de gebruiker geïnitieerde verzoeken tot annulering van bestellingen. Zie [De configuratie van een winkel opvragen voor het annuleren van bestellingen](https://developer-stage.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/#query-a-stores-order-cancellation-configuration)
...
* `GET /V1/products/attributes/{attributeCode}/is-filterable`. Parameters pad: `attributeCode` (`String`).<!-- ACP2E-2275 -->
...
* De onnodige geheim voorgeheugenmanipulatie is verwijderd uit het opstellingsproces. Voorheen schreef Magento Open Source de configuratie onnodig naar schijf wanneer `bin/magento setup:db-data:upgrade` of `bin/magento setup:upgrade` werd uitgevoerd, wat problemen veroorzaakte met som-modules tijdens de installatie. [GitHub-38124](https://github.com/magento/magento2/issues/38124)
...
* `bin/magento setup:install` Wordt nu voltooid nadat `app/etc/config.php` het is verwijderd. Voorheen werd het ontbrekende bestand niet opnieuw gegenereerd tijdens de installatie en gaf Magento Open Source een foutmelding. [GitHub-37805](https://github.com/magento/magento2/issues/37805)
...
* In de afrekenworkflow wordt niet langer een waarschuwing weergegeven wanneer een klant een postcode voor Griekenland invoert die geen lege ruimte bevat. [GitHub-36676](https://github.com/magento/magento2/issues/36676)

<!-- AC-9357 -->

* De beheerderslink `It's time to change your password` verwijst nu zoals verwacht om naar de pagina Beheerderswachtwoord wijzigen. [ GitHub-37902 ](https://github.com/magento/magento2/issues/37902)
...
* De winkelwagenbelasting en verzendcalculator geven nu nauwkeurig de standaardbestemmingsconfiguratie weer. Eerder, toen u de standaardinstellingen voor de berekening van de belastingbestemming configureerde en Staat/Regio en ZIP opgaf, werd alleen de ZIP-waarde weergegeven in de winkelwagen onder **[!UICONTROL Estimate Tax and Shipping]**. Dit was het gevolg van Magento Open Source die de regio-ID instelde op undefined voordat de voorwaardelijke instructie werd aangeroepen die de standaard regio-ID van de beheerder instelde.

<!-- ACP2E-1547 -->

* Magento Open Source genereert deze foutmelding niet langer tijdens het afrekenen wanneer persistent winkelwagentje is ingeschakeld: `Invalid state change requested`. Voorheen, wanneer deze fout werd gegenereerd, werden er af en toe meerdere bestellingen met hetzelfde `quote_id` gemaakt.

<!-- ACP2E-1456 -->

* De minikar geeft nu de prijzen van bundelproducten weer die de configuratie-instellingen van de winkelwagenbelasting weerspiegelen. Voorheen was in de minikar altijd btw inbegrepen in de prijzen van bundelproducten.

<!-- AC-7503 -->

* De geschatte verzendkosten op de winkelwagenpagina zijn nu nauwkeurig. `collectShippingRates` wordt nu nog maar één keer ingesteld. Voorheen werd de schatting van de verzendkosten gedupliceerd. [ GitHub-36648 ](https://github.com/magento/magento2/issues/36648)
...
* Vimeo-video&#39;s worden nu zoals verwacht afgespeeld op productpagina&#39;s in mobiele weergave in op Chromium gebaseerde webbrowsers.

<!-- ACP2E-1959 -->

* De `bin/magento catalog:images:resize` opdracht genereert nu correct productafbeeldingen voor de producten die zijn toegewezen aan een aangepaste website met een aangepast thema.

<!-- ACP2E-1982 -->

* Metagegevens met `NULL` of-waarden `NOT NULL` worden nu zoals verwacht opgeslagen voor het standaardarchief volgens het **selectievakje Standaardwaarde** gebruiken. Voorheen werden metagegevens met `NULL` waarde niet opgeslagen als dit selectievakje niet was aangevinkt.

<!-- ACP2E-2093 -->

* Verkopers kunnen nu met succes een bestaande afbeelding verwijderen van de pagina met productdetails en vervolgens een nieuwe afbeelding met dezelfde naam uploaden. Eerder in die omstandigheden werd op de productpagina van de winkel de juiste afbeelding niet weergegeven.
...
* Winkeliers kunnen nu een configureerbaar product selecteren **[!UICONTROL Skip quantity at this time]** en **[!UICONTROL Skip image uploading at this]** configureren zonder dat dit gevolgen heeft voor de bron van de bijbehorende eenvoudige producten. Voorheen resulteerde selectie **[!UICONTROL Skip quantity at this time]** in het verdwijnen van productbronnen.
...
* Couponcodes met een beperkt aantal toepassingen per klant kunnen nu een tweede keer worden gebruikt wanneer de volgorde waarvoor deze voorheen werd gebruikt, mislukt. Voorheen werd de promotiecode niet vrijgegeven wanneer de vorige bestelling werd geannuleerd.
...
* Het aantal records in behandeling in de `cron_schedule` -tabel is verminderd door te voorkomen dat taken in de vorm van een uitsnede proberen overbodige taakvergrendelingen in de vorm van een uitsnijding te verkrijgen. Voorheen probeerden cron-taken sloten te verkrijgen wanneer het te laat was om de taak uit te voeren. Cron-taken werden `cron_schedule` niet gemarkeerd als gemist, ook al waren ze hun geplande tijd al verstreken. Magento Open Source gaf deze fout ook herhaaldelijk weer in het foutenlogboek: `report.WARNING: Could not acquire lock for cron job: indexer_update_all_views`.

<!-- AC-9207 -->

* De fatale fout delen door nul in cron-expressies is opgelost. [ GitHub-37804 ](https://github.com/magento/magento2/issues/37804)

<!-- ACP2E-1792 -->

* Het `sales_clean_quotes` cron-proces is geoptimaliseerd om sneller te werken door een samengestelde index toe te voegen aan `store_id` en `updated_at columns` in de prijstabel. Deze wijziging verbetert de prestaties van cron-taken bij het verwerken van veel offertes.

<!-- AC-7987 -->

* Admin-gebruikers kunnen nu zien welke taak het `bin/magento cron:run` proces momenteel uitvoert. De titel van het momenteel lopende proces bevat nu de groeps- en taaknamen. [ GitHub-34321 ](https://github.com/magento/magento2/issues/34321)
...
* Het `var/log/system.log` bestand bevat `main.INFO` geen foutmeldingen meer. Het logboek van het menu-item is verplaatst van niveau INFO naar DEBUG. Voorheen werd het `var/log/system.log` bestand overspoeld met berichten als deze: `Add of item with id Magento_Theme::design_config was processed … `.

<!-- AC-8809 -->

* Het vooraf laden van Redis mislukt niet meer wanneer `exec()` het onwaar wordt geretourneerd. Verificatie is toegevoegd aan het preloadproces om te voorkomen dat `array_combine` het als tweede argument wordt ontvangen `false` . Als gevolg hiervan blijft het voorladen draaien wanneer `exec()` het terugkeert`false`. [ GitHub-37509 ](https://github.com/magento/magento2/issues/37509)
...
* `Swaziland` is bijgewerkt naar `Eswatini` de hele codebase. [GitHub-37873](https://github.com/magento/magento2/issues/37873)

<!-- ACP2E-2638 -->

* De nieuwe Nicaraguaanse valuta (NIO) is toegevoegd aan de lijst met beschikbare valuta&#39;s.

<!-- AC-10591 -->

* De naam van een variabele is gecorrigeerd in `CatalogRule/Controller/Adminhtml/Promo/Catalog/NewConditionHtml.php`. [GitHub-38093](https://github.com/magento/magento2/issues/38093)
...
* Beeldverhoudingswaarden zijn toegevoegd aan catalogusafbeeldingen en onnodige scripts zijn verwijderd uit het UI-framework. [GitHub-37691](https://github.com/magento/magento2/issues/37691)
...
* `mview_state` De recordstatus geeft nu nauwkeurig de status aan op basis van de activiteit van de changelog wanneer er een slave-databaseverbinding beschikbaar is. Eerder bleven deze records in cloudimplementaties in de status `waiting` staan, zelfs als er geen nieuwe items in de wijziging waren.
...
* Mutex is geïmplementeerd voor bestellingen om race-omstandigheden tijdens updates door gelijktijdige verzoeken te voorkomen. Voorheen zorgden gelijktijdige aanvragen (racevoorwaarden) voor annuleringen van bestellingen voor dubbele vermeldingen in de `inventory_reservation` tabel.
...
* Er zijn kleine updates doorgevoerd in `around` plug-ins. [ GitHub-31443 ](https://github.com/magento/magento2/issues/31443)
...
* Speciale tekens kunnen nu worden gebruikt in naamvelden. Voorheen was het en-teken (&amp;) beperkt in de naamvalidator, waardoor het toevoegen of bewerken van klanten werd geblokkeerd wanneer de klantnaam een en-teken bevatte. [GitHub-38080](https://github.com/magento/magento2/issues/38080)
...
* CMS-inhoudsblokken die emojis bevatten, worden na de emoji niet meer afgekapt wanneer ze in de database worden opgeslagen. Voorheen werd de inhoud afgekapt na de emoji omdat de standaarddatabaseconfiguratie geen tekens van vier bytes ondersteunde.
...
* Klanten kunnen nu met succes inloggen met een nieuw wachtwoord vanaf het ene apparaat nadat ze hun wachtwoord op een ander apparaat hebben gereset.

<!-- ACP2E-1609 -->

* In het veld Admin-klantadres **Staat/Provincie** wordt nu de laatst opgeslagen waarde bewaard zoals verwacht. Voorheen, wanneer u de waarde uit dit veld verwijderde, bleef Magento Open Source de verwijderde waarde weergeven in het **veld Staat/Provincie** . [GitHub-36846](https://github.com/magento/magento2/issues/36846)
...
* De `getCustomerWishlist` query resulteert niet langer in een interne serverfout bij het opvragen van lijsten die configureerbare producten bevatten.

<!-- AC-8966 -->

* De `customerCart` query retourneert nu alle toegepaste kortingen op bundelproducten zoals verwacht. Voorheen werden de totale kortingen die op een bundelproduct werden toegepast, geretourneerd als nul.
...
* De mutatie `addProductsToCart` rapporteert niet langer gerelateerde fouten in `user_errors` . Voorheen werden fouten met betrekking tot de kar opgenomen `user_errors` , samen met de verwachte bedieningsfouten. [GitHub-37908](https://github.com/magento/magento2/issues/37908)

<!-- ACP2E-2119 -->

* Het `products` productfilter voor query&#39;s kan nu zowel gedeeltelijke als volledige overeenkomsten retourneren. Het nieuwe `match_type` kenmerk (PARTIAL, FULL) ondersteunt het opgeven van het overeenkomsttype. Voorheen leverde de query alleen volledige overeenkomsten op.

<!-- ACP2E-2155 -->

* Mutex is toegevoegd aan de `addConfigurableProductsToCart` query om te voorkomen dat race-condities tijdens een update door gelijktijdige verzoeken worden uitgevoerd. Als een winkelier voorheen twee parallelle aanvragen stuurde om hetzelfde configureerbare product aan een winkelwagentje toe te voegen, werden er twee afzonderlijke artikelen met dezelfde product-SKU toegevoegd. [ GitHub-37847 ](https://github.com/magento/magento2/issues/37847)
...
* GraphQL-transactienamen bevatten nu alleen querynamen op het hoogste niveau in de logboeken van New Relic. Voorheen bevatten transactienamen ook secundaire querynamen.

<!-- AC-8623 -->

* De afhandeling van adresfouten voor onderdelen van de GraphQL-bibliotheek werkt nu zoals verwacht.
...
* Importeren van product mislukt niet meer vanwege onvoldoende geheugen. Voorheen mislukte het importeren van een willekeurig aantal producten in een database die al ongeveer tien miljoen producten bevat vanwege een gebrek aan geheugen.

<!-- AC-9116 -->

* Validatiecontroles stoppen nu het importproces zoals verwacht wanneer er geen geldige gegevens zijn om te importeren, en Magento Open Source geeft nu deze fout weer: `There are no valid rows to import`. Voorheen werd de validatie onder deze voorwaarden doorstaan, maar het importproces mislukte met dit bericht: `entity values are mixed`. [GitHub-32905](https://github.com/magento/magento2/issues/32905)

<!-- ACP2E-1754 -->

* Als u URL-sleutels met hetzelfde product importeert, worden bestaande standaard winkelweergavesleutels niet meer overschreven of verwijderd. Het herschrijven van URL&#39;s wordt nu alleen opnieuw gegenereerd voor winkelweergaven als er geen overschreven `url_key` waarde is. Voorheen werd bij het importeren van URL-herschrijvingen met dezelfde URL-sleutel de bestaande standaardURL-sleutel voor winkelweergaven overschreven. `key_store`( URL-herschrijvingen zijn verwijderd, maar de URL-herschrijving op het standaardweergaveniveau voor het product was nog steeds ingesteld op `key_store`.)
...
* Het importeren van producten met `country_of_manufacture` kenmerken die zijn gemaakt door een beheerdersgebruiker met een andere landinstelling dan de beheerdersgebruiker die de producten importeert, genereert geen validatiefout meer. Voorheen kon deze importbewerking resulteren in een validatiefout op het `country_of_manufacture` kenmerk.
...
* Niet-standaardconfiguraties (website of store scope) die worden toegevoegd als omgevingsvariabelen, onderbreken het `app:config:import` proces niet langer met een recursiefout.
...
* Alle indexeerfuncties gebruiken nu dezelfde kleuren om de status van de indexeerfunctie weer te geven. [GitHub-34648](https://github.com/magento/magento2/issues/34648)
...
* `bin/magento indexer:reindex customer_grid` mislukt niet langer met een MySQL-fout wanneer het klantenraster nieuw gemaakte aangepaste klantkenmerken bevat. [GitHub-36233](https://github.com/magento/magento2/issues/36233)
...
* De `POST <store_code>/V1/inventory/source-items` REST-aanroep valideert nu de payload voor witruimte in geretourneerde voorraadbronartikelen en verzendt indien nodig een foutbericht over validatie.

<!-- ACP2E-2072 -->

* Winkeliers kunnen shoppers nu laten weten dat een bestelling klaar is om te worden opgehaald wanneer de **[!UICONTROL No Manage Stock]** instelling is ingeschakeld voor een product. Eerder, toen deze instelling was ingeschakeld, gaf Magento Open Source deze foutmelding weer: `Your order is not ready for pickup`.

<!-- ACP2E-2227 -->

* De handelaren kunnen nu een creditnota voor orden tot stand brengen waarvan een eenvoudig product dat met een configureerbaar product wordt geassocieerd is geschrapt. Voorheen konden verkopers de creditnota niet maken en Magento Open Source maakte een `TypeError` uitzondering.
...
* Admin-gebruikers kunnen nu de logboeken voor bulkacties die zijn gemaakt door integraties bekijken in de sectie Bulkactieslogboek van de beheerder.
...
* De filterfunctie voor aanschaf in de sectie met klantorders op de pagina met bestellingen werkt nu naar behoren. Voorheen trad er een JavaScript-fout op wanneer de logica voor tijdzoneconversie een fout genereerde wanneer dezelfde tijdzone aan de methode was opgegeven.

<!-- ACP2E-1294 -->

* Op het dashboard Admin worden nu de juiste volgordestatistieken weergegeven bij de eerste keer laden. Voorheen gaf het dashboard onjuiste bestelinformatie weer, maar werd de juiste informatie weergegeven zodra de tijdsperiode was bijgewerkt.

<!-- ACP2E-1762 -->

* Admin-gebruikers worden nu zoals verwacht omgeleid naar de bestelpagina nadat ze een winkelweergave hebben geselecteerd tijdens het maken van een nieuwe bestelling vanuit de beheerder.

<!-- ACP2E-1418 -->

* Het `GET V1/orders/<OrderID>` eindpunt retourneert nu informatie over zowel de configureerbare als de eenvoudige producten in de order, ongeacht de voorraadstatus van de eenvoudige producten. Admin-gebruikers kunnen nu alleen bestellingen plaatsen als de geselecteerde artikelen (producten) op voorraad of verkoopbaar zijn. Voorheen gaf deze API alleen informatie terug over de producten die op voorraad waren.

<!-- AC-7901 -->

* De pagina met creditnota&#39;s loopt niet meer vast wanneer gratis verzending is ingeschakeld, wanneer de verzendprijs inclusief btw is en belastingberekeningen worden toegepast na kortingen. Eerder had Magento Open Source een uitzondering van het type `Division by zero` . [ GitHub-36800 ](https://github.com/magento/magento2/issues/36800)
...
* Het ongebruikte `Magento\Catalog\Controller\Adminhtml\Product\Initialization\Helper` verwijderd uit `Magento\Catalog\Model\ProductRepository`. Eerder heeft u deze helper, die een `Adminhtml` controller is, `ProductRepository` geladen. [ GitHub-37864 ](https://github.com/magento/magento2/issues/37864)

<!-- AC-9127 -->

* Verbeterde prestaties van de laadsnelheid van pagina&#39;s wanneer veel opties worden toegevoegd aan gebundelde producten. [ GitHub-29409 ](https://github.com/magento/magento2/issues/29409)

<!-- ACP2E-2069 -->

* Het proces van het toewijzen van producten aan categorieën vanaf het **[!UICONTROL Products in Category]** tabblad is geoptimaliseerd. De **[!UICONTROL Assign]** knoppen en **[!UICONTROL Remove]** op het **[!UICONTROL Add Product by SKU]** tabblad zijn nu uitgeschakeld totdat producten volledig in het raster zijn geladen. Voorheen konden beheerders geen producten per SKU toevoegen aan catalogi die miljoenen geregistreerde producten bevatten.

<!-- ACP2E-2098 -->

* De prestaties van de `addProductToCart` mutatie bij het uitvoeren met veel aangepaste productattributen zijn verbeterd. Eerder, toen deze mutatie werd uitgevoerd met veel productattributen (ongeveer 1000), probeerde Magento Open Source alle productattributen te laden.
...
* Verbeterde berichtgeving van fouten die optreden tijdens het renderen van sjablonen. Magento Open Source geeft nu een informatieve boodschap weer. Eerder gaf Magento Open Source HTML-lekken weer toen er een PHP-fout optrad.

<!-- ACP2E-2595 -->

* Aangepaste klantkenmerken (type `File attached`) werken nu correct met afbeeldingen die zijn verwijderd en vervolgens zijn geüpload. Voorheen konden aangepaste kenmerken die werden geüpload, vervolgens verwijderd en vervolgens opnieuw geüpload, niet worden opgeslagen vanuit de etalage.

<!-- AC-9125 -->

* Er is een syntaxisfout in het kalendersjabloonbestand gecorrigeerd. [ GitHub-37725 ](https://github.com/magento/magento2/issues/37725)
...
* Admin-gebruikers kunnen nu de aanpasbare opties die voor het eenvoudige product zijn gemaakt, slepen en neerzetten, zoals verwacht. [ GitHub-36051 ](https://github.com/magento/magento2/issues/36051)
...
* Welkomstberichten met enkele aanhalingstekens worden nu correct weergegeven in de etalage zonder consolefouten. Als een welkomstbericht voorheen één aanhalingsteken bevatte, werd het welkomstbericht niet geladen tijdens het renderen van de winkel. Magento Open Source gaf ook consolefouten weer.

<!-- ACP2E-1369 -->

* Magento Open Source geeft niet langer een foutmelding wanneer u snel de weergave probeert te wijzigen tussen reactiegeschiedenis en Retouren op de pagina Admin-volgorde. Voorheen gaf Magento Open Source de volgende algemene fout weer: `A technical problem with the server created an error`.

<!-- ACP2E-1374 -->

* Beheerders met bevoegdheden die beperkt zijn tot één website, kunnen nu websitespecifieke productkenmerken bijwerken die zijn ingesteld op een `all stores view` bereik.

<!-- ACP2E-1648 -->

* De vergrootglaswidget laadt nu zoals verwacht op de productpagina.

<!-- ACP2E-1216 -->

* Aanpasbare opties met meerdere regels worden nu correct weergegeven in PDF-documenten voor facturen, verzendingen en creditnota&#39;s. Eerder werd tekst met meerdere regels voorafgegaan door een onverwacht pagina-einde en als de tekst de pagina overschrijdt, werd de tekst afgekapt.
...
* CMS-pagina&#39;s voor meerdere winkels zonder URL-herschrijving worden nu doorgestuurd zoals u had verwacht. Voorheen werden aangevraagde CMS-pagina&#39;s met meerdere winkels zonder URL-herschrijvingen omgeleid naar de startpagina van de winkel, maar de oorspronkelijke CMS-pagina&#39;s werden niet omgeleid.

### Web API-raamwerk

<!-- AC-6980 -->

* Magento Open Source geeft niet langer een `Integration not secure` waarschuwing weer in Admin **[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** wanneer de **[!UICONTROL Callback URL]** velden en **[!UICONTROL Identity link URL]** leeg zijn. [GitHub-36407](https://github.com/magento/magento2/issues/36407)

<!-- ACP2E-2483 -->

* De `taxes`attributen , `itemized_taxes`, en `additional_itemized_taxes` extensie zijn toegevoegd aan `Magento\Sales\Api\Data\OrderInterface`. Deze attributen staan u toe om toegepaste belastingen met de vraag van de TerugSTEL van de Orde te bewaren.

<!-- ACP2E-2610 -->

* De REST-API voor niveauprijzen ondersteunt nu decimale hoeveelheden. Voorheen stond deze API geen decimale hoeveelheden toe, zelfs niet als de **[!UICONTROL Use Decimal Quantity]** instelling was ingeschakeld.

<!-- AC-8998 -->

* Het eindpunt REST `V1/orders/{id}/comments` verandert niet meer de status van een orde wanneer het proberen om een commentaar aan de status van een hangende orde toe te voegen. Het geeft nu een informatieve fout. Voorheen veranderde dit eindpunt de orderstatus bij het toevoegen van een opmerking.

<!-- ACP2E-2275 -->

* De REST API `GET` en `POST V1/products/attributes` het eindpunt retourneren niet langer dezelfde waarde voor het `is_filterable` kenmerk voor beide `Filterable(with results)` en `Filterable(no results)` opties van de **[!UICONTROL Use in Layered Navigation]** optie. Voorheen retourneerde dit eindpunt dezelfde waarde omdat de eigenschap van het `is_filterable` kenmerk van het type `Boolean`was, waardoor deze eigenschap `Filterable(no results)`niet op . [GitHub-37979](https://github.com/magento/magento2/issues/37979)
...
U kunt Magento Open Source 2.4.7 installeren gebruikend [ Composer ](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html?lang=nl-NL).
