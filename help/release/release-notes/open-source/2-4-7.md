...
* [tweeledige verificatie](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication)
* gebruik van VPN
...
* **RabbitMQ 3.13 steun**. Deze release is compatibel met de nieuwste versie van RabbitMQ 3.13. De compatibiliteit blijft behouden met RabbitMQ 3.11 en 3.12, die tot respectievelijk augustus 2024 en december 2024 worden ondersteund, maar Adobe beveelt aan Magento Open Source 2.4.7 alleen te gebruiken met RabbitMQ 3.13. <!-- AC-9659 -->

* **Composer 2.7.x**. Compatibiliteit met Composer 2.2.x blijft behouden. <!-- AC-9663 -->

* **Varnish geheime voorgeheugen 7.4 steun**. Deze release is compatibel met de nieuwste versie van Varnish Cache 7.4. De compatibiliteit blijft behouden met de 6.0.x- en 7.2.x-versies, maar we raden u aan Magento Open Source 2.4.7 alleen te gebruiken met Varnish Cache versie 7.4 of versie 6.0 LTS. <!-- AC-9662 -->
...
* [Adobe Commerce-integratie met Adobe IMS](https://experienceleague.adobe.com/docs/commerce-admin/start/admin/ims/adobe-ims-integration-overview.html?lang=nl-NL)
...
* Magento Open Source meldt deze fout niet meer tijdens het afrekenen wanneer persistente winkelwagentjes zijn ingeschakeld: `Invalid state change requested`. Eerder, toen deze fout werd geworpen, werden de veelvoudige orden met het zelfde `quote_id` af en toe gecreeerd.
...
* Gast-API-aanvragen voor `POST V1/guest-carts/<cartId>/shipping-information` retourneren niet langer de status `cancel` als de pagina tijdens de aanmelding in een mobiele omgeving opnieuw wordt geladen. Eerder, wierp Magento Open Source een fout 500 en registreerde deze uitzondering: `TypeError: Argument 2 passed to Magento\CustomerCustomAttributes\Model\Plugin\ProcessCustomerShippingAddressCustomAttributes::beforeSaveAddressInformation() must be of the type string, null given`.
...
* Magento Open Source genereert deze uitzondering op het bestandssysteem niet meer wanneer u de JavaScript/CSS-cache probeert te leegmaken: `No such file or directory`.
...
* Magento Open Source genereert niet langer een fout bij het maken van een tweede gedeeltelijke factuur voor een bestelling die door Braintree wordt betaald wanneer **[!UICONTROL Enable Vault for Card Payments]** is ingeschakeld. Eerder, toen een handelaar probeerde een tweede factuur te creëren, gaf Magento Open Source deze fout: `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.
...
* Foutberichten die zijn gekoppeld aan het verzenden van opmerkingen op de pagina met orderdetails, zijn nu informatiever. Eerder, toen een verkoopster **[!UICONTROL Submit comment]** klikte zonder of ordestatus te veranderen of tekst in het commentaargebied in te gaan, toonde Magento Open Source dit bericht: `The comment is missing. Enter and try again` .
...
* Asynchrone bulksgewijs REST API-bewerkingen om producten met laagprijzen en verschillende kenmerksets bij te werken zijn nu voltooid. Eerder konden producten met verschillende kenmerksets niet op deze manier worden bijgewerkt. Deze fout werd door Magento Open Source weergegeven: `The product was unable to be saved. Please try again`.
