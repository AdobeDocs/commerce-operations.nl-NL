---
title: Opmerkingen bij de release Magento Open Source 2.4.6
description: Meer informatie over nieuwe functies, verbeteringen, foutoplossingen en bekende problemen vindt u in de release 2.4.6 Magento Open Source.
...
# Opmerkingen bij de release Magento Open Source 2.4.6
...
* OpenSearch wordt nu ondersteund als de standaardzoekengine voor Magento Open Source-implementaties op locatie en cloud. De functie wordt ondersteund als een onafhankelijke zoekfunctie voor Magento Open Source. Deze release ondersteunt OpenSearch v2.x en is getest met OpenSearch 2.5. Hoewel compatibiliteit met OpenSearch 1.x overblijft, raden we u aan deze release te gebruiken met OpenSearch 2.x. U kunt Magento Open Source en Magento Open Source 2.4.6 installeren en uitvoeren met OpenSearch 2.x op zowel Cloud als op het terrein [Voorwaarden voor zoekmachines](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html). <!-- AC-449 3555 -->
...
* De compatibiliteit met PHP 7.4 is verwijderd uit de code base of Commerce en Magento Open Source. <!-- AC-2841-->
...
* `jquery-migrate` is verwijderd uit de grondslagen voor de handels- en Magento Open Source-code. [GitHub-21406](https://github.com/magento/magento2/issues/21406) <!-- AC-1199 -->
...
* Bijgewerkt `web-token/jwt-framework` tot en met v3.0.5. (meest recente versie) <!-- AC-3772-->
...
De volgende Adobe IMS-modules zijn losgekoppeld van de Adobe Commerce- en Magento Open Source-kerncode en zijn nu verpakt als een apart pakket:
...
**Probleem**: New Relic is momenteel niet compatibel met PHP 8.2. Implementaties met New Relic kunnen Adobe Commerce 2.4.6 uitvoeren op PHP 8.1, maar niet PHP 8.2. **Workaround**: Voer Adobe Commerce 2.4.6 uit op PHP 8.1 totdat een nieuwe versie van New Relic wordt uitgebracht die PHP 8.2 ondersteunt. Deze release wordt verwacht in de zomer van 2023. <!-- AC-7769-->

## Opgeloste problemen

We hebben honderden problemen opgelost in de kerncode Magento Open Source 2.4.6.
...
* Magento Open Source berekent nu correct het totaal aan kosten voor terugbetalingen van bestellingen met een korting voor verzending.

<!-- ACP2E-876-->

* Google reCAPTCHA werkt nu zoals verwacht voor orders van winkels. Eerder gaf Magento Open Source deze fouten weer toen Google ReCAPTCHA niet voor de eerste keer was geselecteerd voordat een winkel een bestelling probeerde te plaatsen: `ReCaptcha validation failed, please try again` en `No such cart with id = ID`.
...
* Magento Open Source toont nu een bevestigingspop-up zoals verwacht alvorens een verkoopster aan de volgende pagina opnieuw te richten wanneer de verkoopster producthoeveelheid op de kartonpagina verandert zonder de karretje bij te werken en dan aan de volgende pagina probeert te navigeren.

<!-- AC-5987-->

* Klanten kunnen nu de hoeveelheid van een product uit hun winkelwagentje verlagen nadat het in de Admin-service is verlaagd. Eerder, gooide Magento Open Source de volgende fout toen een verkoopster probeerde om producthoeveelheid te verminderen: `The requested qty is not available`. [GitHub-35780](https://github.com/magento/magento2/issues/35780)
...
* Magento Open Source geeft niet langer een fout weer op de winkelwagentje als u een product in een hoeveelheid toevoegt die groter is dan de voorraad.
...
* Magento Open Source geeft niet langer gegevens van de vorige pagina weer wanneer u een productdetailpagina opnieuw laadt. Eerder, toen de productdetailpagina opnieuw werd geladen, werden de gegevens van het eerste lusje samengevoegd met de onlangs geladen gegevens voor een gespleten seconde.
...
* Magento Open Source genereert niet langer een fout wanneer een admin-gebruiker handmatig producten toevoegt aan een configureerbaar product waaraan al veel eenvoudige producten zijn toegewezen. Eerder gaf Magento Open Source deze fout weer: `Something went wrong with processing the default view and we have restored the filter to its original state`.
...
* Magento Open Source verzendt geen dubbele e-mailberichten meer van de klant nadat een bestelling van de beheerder is gemaakt.
...
* AMQP-consumenten falen niet meer vanwege een verouderde `trim()` functie voor implementaties met PHP 8.1. Eerder, toen de consument AMQP na installatie begon, gaf Magento Open Source deze fout: `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in .../vendor/magento/framework-amqp/Config.php on line 143`. [GitHub-35380](https://github.com/magento/magento2/issues/35380)

<!-- AC-2882-->

* De `getFilteredTimerIds` Deze methode werkt nu zoals u had verwacht.  Klasse `Magento\Framework\Profiler\Driver\Standard\Stat` is nu compatibel met PHP 8.1. Eerder gaf Magento Open Source een uitzondering toen deze methode werd aangeroepen. [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* Magento Open Source geeft nu een nauwkeurig tijdbereik weer wanneer een beheerder de optie **[!UICONTROL Last 24 Hours]** in het dashboard voor beheerdersstatistieken. Eerder gaf Magento Open Source een onjuist tijdbereik weer. Dit gebeurde omdat de tijdzone van het gegevensbestand onveranderd bleef.
...
* Gebruikers met beheerdersrechten kunnen nu een bestelling maken of opnieuw rangschikken wanneer de betalingsmethode Braintree is ingeschakeld. Eerder heeft Magento Open Source deze fout gegenereerd: `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`.

<!-- BUNDLE-3139-->

* Afhandeling met Braintree-betalingsmethode wordt nu voltooid wanneer Braintree PayPal is ingeschakeld en **[!UICONTROL Enable Card Payments]** is uitgeschakeld. Eerder werd de voortgangsbalk nooit opgelost en gaf Magento Open Source deze consolefout weer: `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`. [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* Gebruikers met beheerdersrechten kunnen nu een bestelling of een nieuwe volgorde voor klanten maken wanneer de betalingsmethode Braintree is ingeschakeld. Eerder heeft Magento Open Source deze fout gegenereerd: `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`

<!-- BUNDLE-3139-->

* Klanten kunnen nu de betalingsmethode Braintree gebruiken in winkels waar **[!UICONTROL Enable Card Payments]** is uitgeschakeld. Eerder heeft Magento Open Source deze JavaScript-fout gegenereerd: `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* Zoekopdrachten naar tekenreeksen binnen aanhalingstekens werken nu naar behoren wanneer de optie **[!UICONTROL Minimum Terms to Match]** configuratie-instelling is ingeschakeld. Eerder heeft Magento Open Source deze fout gegenereerd: `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* Magento Open Source genereert niet langer een fout met het minimale orderbedrag wanneer een korting wordt toegepast op de verzendkosten.
...
* Magento Open Source kan nu gegroepeerde producten met belasting weergeven op de pagina met winkelcategorieën in implementaties met PHP 8.1. Eerder gaf Magento Open Source geen lijst van producten weer en gooide deze uitzondering weg: `main.CRITICAL: Exception: Deprecated Functionality: ucfirst(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/m24/vendor/magento/module-tax/Pricing/Render/Adjustment.php on line 188 in /var/www/m24/vendor/magento/framework/App/ErrorHandler.php:61`. [GitHub-35500](https://github.com/magento/magento2/issues/35500)
...
* De filterfunctie voor het gebruikersrolraster (Admin **[!UICONTROL System]** > **[!UICONTROL Permissions]** > **[!UICONTROL User roles]** > **[!UICONTROL Administrators]**) werkt nu zoals verwacht. De arraywaarde wordt nu afzonderlijk afgehandeld. Eerder, toen u probeerde om een vraag in dit net in te gaan, gooide Magento Open Source deze consolefout, en de pagina niet laadde: `Uncaught TypeError: Cannot read properties of null`. [GitHub-35993](https://github.com/magento/magento2/issues/35993)
...
* De Fixer-valutaserservice werkt nu zoals u had verwacht met sleutels die via ApiLayer zijn gegenereerd en de valutakoersen worden correct geëxporteerd. Eerder gaf Magento Open Source een fout op toen een handelaar via de Admin probeerde valutakoersen te importeren.
...
**Probleem**: Je kunt geen creditmemo maken als gratis verzending is ingeschakeld (**[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL Sales]** > **[!UICONTROL Delivery Methods]**) en de weergegeven verzendprijs inclusief btw. Wanneer u een creditmemo probeert te maken, wordt deze uitzondering gegenereerd: `"Division by zero".vendor/magento/module-sales/Model/Order/Creditmemo/Total/Tax.phpLine 139`. [GitHub-36800](https://github.com/magento/magento2/issues/36800)

**Workaround**: Er is nu een hotfix beschikbaar voor dit probleem. Ga naar [Gereedschap Kwaliteitspatches: Zoeken naar patches](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) en zoek naar patch ACSD-50814.
