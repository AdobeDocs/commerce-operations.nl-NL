...
Deze release bevat verbeteringen aan de kernkwaliteit, die de kwaliteit van het framework en de volgende functionele gebieden verbeteren: Klantenaccount, Catalogus, CMS, OMS, Importeren/exporteren, Promoties en Targeting, Winkelwagentje en Afhandeling, B2B, en Staging en Voorvertoning.
...
Zie ](https://experienceleague.adobe.com/en/docs/commerce-admin/config/general/b2b-features) Eigenschappen 0} B2B.[
...
* [Yotpo-productherzieningen](https://commercemarketplace.adobe.com//yotpo-module-yotpo.html)
...
* De toepassing genereert niet langer een validatiefout wanneer u POST `/V1/products` gebruikt voor een configureerbaar product met een `int` -waarde van 0. Eerder heeft de toepassing deze fout gegenereerd: `Product with id "%1" does not contain required attribute "%2"."` _Repareren die door Vadim Malesh in trekkrachtverzoek [ wordt voorgelegd 29001 ](https://github.com/magento/magento2/pull/29001)_. [ GitHub-13210 ](https://github.com/magento/magento2/issues/13210)
...
* `cron` -expressies zoals `3/10 * * * *` worden nu volgens planning uitgevoerd. _Reparatie die door Anton Evers in trekkrachtverzoek [ wordt voorgelegd 28930 ](https://github.com/magento/magento2/pull/28930)_. [ GitHub-29240 ](https://github.com/magento/magento2/issues/29240)

<!-- ENGCOM-7193-->

* Met `sales_clean_quotes` worden niet langer alle verlopen aanhalingstekens tegelijk geladen. Eerder is deze fatale fout opgetreden in de toepassing omdat alle verlopen aanhalingstekens tegelijk zijn geladen: `PHP Fatal error: Allowed memory size of 2147483648 bytes exhausted (tried to allocate 20480 bytes) in /path/to/magento2/vendor/magento/framework/Model/AbstractModel.php on line 359` ,
...
* Het leven van het wachtwoord zoals die in **wordt geplaatst Slaat** > **Configuratie** > **Geavanceerd** > **Admin** wordt nu geëerd. Eerder, als u **het vergeten wachtwoord klikte?** als u wordt gevraagd uw wachtwoord opnieuw in te stellen, kunt u de wachtwoordinstelling omzeilen.
...
* De configuratie UI van de Galerij van Media (Admin **Slaat** > **Configuratie** > **Geavanceerd** > **Systeem**) is gereorganiseerd. _Reparatie die door Shankar Konar in trekkingsverzoek [ wordt voorgelegd 29433 ](https://github.com/magento/magento2/pull/29433)_. [ GitHub-28011 ](https://github.com/magento/adobe-stock-integration/issues/1738)
...
* De actiegroepen `AdminSubmitAdvancedInventoryFormActionGroup` , `AdminClickOnAdvancedInventoryLinkActionGroup` en `AdminSetStockStatusConfigActionGroup` worden nu gebruikt in tests volgens de aanbevolen procedures. _Reparatie die door Oleh USIA in trekkingsverzoek [ wordt voorgelegd 29386 ](https://github.com/magento/magento2/pull/29386)_. [ GitHub-29420 ](https://github.com/magento/magento2/issues/29420)
...
* De titel van de pagina voor het mislukken van de bestelling is voor de nauwkeurigheid herschreven. Eerder, toen een verkoopster een bestelling annuleerde, toonde de toepassing een pagina met deze titel: `We received your order!`. _Reparatie die door Angelo Romano in trekkingsverzoek [ wordt voorgelegd 29410 ](https://github.com/magento/magento2/pull/29410)_. [ GitHub-29416 ](https://github.com/magento/magento2/issues/29416)
...
* Aan adressen gerelateerde extensiekenmerken kunnen worden ingesteld als gegevensarrays. Eerder genereerde de toepassing een fout toen een POST-aanvraag met `extension_attributes` naar `estimate-shipping-methods` werd verzonden. Deze bug, die werd geïntroduceerd door een eerdere commit, beïnvloedde alle extensiekenmerken die zijn ingesteld op `Magento\Quote\Api\Data\AddressInterface` en geen objecten waren. _Reparatie die door Alexander Menk in trekkingsverzoek [ wordt voorgelegd 27338 ](https://github.com/magento/magento2/pull/27338)_. [ GitHub-26682 ](https://github.com/magento/magento2/issues/26682)
...
* U kunt nu de zoekfunctie voor lijsten met wensen gebruiken om een product te zoeken in een lijst met openbare wensen in implementaties waar ondersteuning voor meerdere lijsten met wensen is ingeschakeld. Eerder, nadat een klant het onderzoek van de verlanglijst gebruikte om een product te vinden, het te selecteren, en het klikken **toevoegde aan wagentje**, voegde de toepassing niet het product aan het karretje toe en toonde deze fout: `Invalid Form Key. Please refresh the page` .
...
**Uitgave**: De nieuwe CAPTCHA-functie voor afrekenen werkt niet zoals verwacht op de pagina Place Order wanneer u betalingsproviders van derden gebruikt. Handelaren met 2.3.6 of 2.4.1 die CAPTCHA-beveiliging op de winkelpagina Place Order hebben ingeschakeld, zien deze fout wanneer ze uitchecken via een externe betalingsprovider, zoals PayPal: `Please provide CAPTCHA code and try again`. **Oplossing**: Er is nu een oplossing voor dit probleem beschikbaar. Zie [ Adobe Commerce v2.3.6/2.4.1 CAPTCHA in controle die ](https://support.magento.com/hc/en-us/articles/360051235772) het artikel van de Kennisbank niet werkt. Een oplossing wordt ook opgenomen in onze volgende driemaandelijkse patch (Q12021).

**Uitgave**: Gebruikers zonder beheerdersrechten kunnen momenteel hun persoonlijke 2FA-toegang niet instellen. 2FA omvat twee ACL rollen. Één rol beïnvloedt globale systeemconfiguratie en het is nodig slechts wanneer het vormen van het systeem. De tweede ACL rol beïnvloedt individuele gebruiker 2FA rekeningen. Een admin gebruiker moet dit tweede type van 2FA ACL vormen. **Oplossing**:  Nadat de gebruiker het programma geopend en de Toegang ontkende scherm heeft gezien, kunnen zij `https://<magento store>/<admin_path>/tfa/tfa/requestconfig/` bezoeken om configuratie te dwingen. Opmerking: We raden u niet aan de beveiligingsinstellingen uit te schakelen. Deze tussenoplossing is echter alleen effectief wanneer de geheime sleutels voor Admin URL zijn uitgeschakeld.

**Uitgave**: **creeer een knoop van de Rekening** op de Create Nieuwe pagina van de Rekening blijft gehandicapt als een verkoopster ongeldige gegevens heeft ingegaan. Zo voorkomt u dat kopers opnieuw proberen een account te maken nadat ze een fout hebben gemaakt. **Oplossing**: patch toepassen `MC-38509` . Een oplossing zal ook in onze volgende driemaandelijkse versies (2.4.2, 2.4.1-p1 en 2.3.6-p1) worden opgenomen, die voor versie in Q1 2021 gepland zijn. Zie [ 2.4.1 en 2.3.6 tot een van de rekeningsknoop gehandicapte hotfix ](https://support.magento.com/hc/en-us/articles/360051130212) artikel van de Kennisbank leiden.  [ GitHub-30513 ](https://github.com/magento/magento2/issues/30513)

**Uitgave**: Merchants kunnen zich niet bij dotdigital aanmelden bij Admin in Safari wanneer dotdigital is ingeschakeld. Zie [ het is onmogelijk aan login in dotdigital via admin paneel wanneer dotdigital account ](https://support.magento.com/hc/en-us/articles/360050092291) het artikel van de Kennisbank wordt toegelaten.

**Uitgave**: Validatie van hoekpuntadressen werkt niet tijdens betaling wanneer de gebruiker een verzendadres selecteert dat afwijkt van het factuuradres. De kwestie wordt in punt 2.4.2 geregeld. Zie [ 2.4.1 gekende kwestie: Het bericht van de Bevestiging van het Adres van de top verdwijnt niet na het bijwerken van het adres ](https://support.magento.com/hc/en-us/articles/360050139631) artikel van de Kennisbank.

**Uitgave**: De toepassing geeft een lege pagina weer wanneer een handelaar een DTP-formulier opslaat in de Safari-browser. **Oplossing**: Laad de pagina opnieuw om de wijzigingen toe te passen. Zie [ 2.4.1 gekende kwestie: lege pagina na het opslaan van het dotdigital Page Builder formulier ](https://support.magento.com/hc/en-us/articles/360049819092) Knowledge Base-artikel.

**Uitgave**: Onder bepaalde omstandigheden worden klanten automatisch aangemeld bij eerder gebruikte Amazon-accounts, in plaats van dat ze worden gevraagd zich aan te melden bij de afhandeling met Amazon Pay. Zie [ 2.4.1 gekende kwestie: kan Amazon-account niet wijzigen in Google Chrome ](https://support.magento.com/hc/en-us/articles/360049814152) Knowledge Base-artikel.

**Uitgave**: De toepassing geeft kort een foutbericht weer op de factureringspagina van de afhandelingsworkflow wanneer PayPal Braintree betaling en meerdere adressen voor verzending zijn geselecteerd. Zie [ 2.4.1 gekende kwestie: fout die op Betaling met PayPal Braintree ](https://support.magento.com/hc/en-us/articles/360050253211) het artikel van de Kennisbank opduikt.

**Uitgave**: De toepassing geeft een niet-informatief foutbericht weer wanneer een gast een bestelling probeert te plaatsen met PayPal via Braintree wanneer uitchecken door gasten is uitgeschakeld. Zie [ 2.4.1 gekende kwestie: niet-informatieve foutenmelding tijdens gastcontrole gebruikend PayPal door het artikel van de Kennisbank van Braintree ](https://support.magento.com/hc/en-us/articles/360050368111).
