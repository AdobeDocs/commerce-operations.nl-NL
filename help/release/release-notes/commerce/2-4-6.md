...
Deze release bevat meer dan 300 correcties en verbeteringen voor kwaliteit. De afhankelijkheden van Core Composer en bibliotheken van derden zijn bijgewerkt naar de nieuwste versies die compatibel zijn met PHP 8.2.
...
* OpenSearch wordt nu ondersteund als de standaard zoekengine voor Adobe Commerce-implementaties op locatie en in de cloud. De functie wordt ondersteund als een onafhankelijke zoekfunctie voor Magento Open Source. Deze release ondersteunt OpenSearch v2.x en is getest met OpenSearch 2.5. Hoewel compatibiliteit met OpenSearch 1.x overblijft, raden we u aan deze release te gebruiken met OpenSearch 2.x. U kunt Adobe Commerce en Magento Open Source 2.4.6 installeren en uitvoeren met OpenSearch 2.x op zowel Cloud als op het terrein [Voorwaarden voor zoekmachines](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html). <!-- AC-449 3555 -->
...
* `jquery-migrate` is uit de handelsschulden verwijderd. [GitHub-21406](https://github.com/magento/magento2/issues/21406) <!-- AC-1199 -->
...
* Het schema Purchase Order GraphQL omvat de functionaliteit van het beheer van inkooporders van B2B-bedrijven, zoals het maken, bewerken en verwijderen van inkooporders. Zie [Inkooporders (B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order/).
* Met het schema Purchase Order Rule GraphQL kunt u goedkeuringsregels maken, bewerken en verwijderen en inkooporders handmatig valideren. Zie [Goedkeuringsregels voor inkooporders (B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order-rule/).
...
* Klanten kunnen nu de hoeveelheid van een product uit hun winkelwagentje verlagen nadat het in de Admin-service is verlaagd. Eerder had Adobe Commerce de volgende fout gegenereerd toen een winkel de hoeveelheid product probeerde te verlagen: `The requested qty is not available`. [GitHub-35780](https://github.com/magento/magento2/issues/35780)
...
* Adobe Commerce geeft nu een informatief bericht wanneer een `addProductsToCart` mutatie mislukt wanneer een product met onvoldoende rechten aan de wagen wordt toegevoegd.
...
* Verwante producten worden nu met succes opgehaald op de detailpagina van het winkelproduct wanneer zij door de verwante productregel voor alfanumerieke productkenmerken worden geproduceerd. [GitHub-36285](https://github.com/magento/magento2/issues/36285)
...
* De `getFilteredTimerIds` methode werkt nu zoals verwacht.  Klasse `Magento\Framework\Profiler\Driver\Standard\Stat` is nu compatibel met PHP 8.1. Eerder gaf Adobe Commerce een uitzondering toen deze methode werd aangeroepen. [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* GitHub- uitgiftesjablonen zijn geconverteerd naar de uitgifte van formulieren voor de ervaring van ontwikkelaars en voor functieverzoeken.
...
* De verouderde medialerie is nu standaard uitgeschakeld. Eerder **[!UICONTROL Enable old Media gallery]** is ingesteld op **[!UICONTROL Yes]**.
...
* `Dropdown`, `Text area`, en `Text field` kenmerken zijn nu beschikbaar in de lijst met productkenmerken van de Page Builder. Alleen voorheen `Dropdown` en `Text area` er waren kenmerken beschikbaar.
...
* Afhandeling met Braintree-betalingsmethode wordt nu voltooid wanneer Braintree PayPal is ingeschakeld en **[!UICONTROL Enable Card Payments]** is uitgeschakeld. Eerder werd de voortgangsbalk nooit opgelost en gaf Adobe Commerce deze consolefout weer: `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`. [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* Klanten kunnen nu de betalingsmethode Braintree gebruiken in winkels waar **[!UICONTROL Enable Card Payments]** is uitgeschakeld. Eerder heeft Adobe Commerce deze JavaScript-fout gegenereerd: `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* Het beheerlograpport bevat nu een nauwkeurige weergave **[!UICONTROL Value before change]** waarde wanneer een categorieregel wordt bijgewerkt.
...
* Zoekopdrachten naar tekenreeksen binnen aanhalingstekens werken nu naar behoren wanneer de optie **[!UICONTROL Minimum Terms to Match]** configuratie-instelling is ingeschakeld. Eerder heeft Adobe Commerce deze fout gegenereerd: `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* Automatische groepstoewijzing werkt nu wanneer er geen BTW-identificatienummer aan het adres van de klant is gekoppeld. Als het adres geen BTW-identificatienummer bevat, wordt het beschouwd als een niet-BTW-geregistreerde afnemer en wijzigt Adobe Commerce de klantengroep in een niet-BTW-groep. Eerder, wanneer **[!UICONTROL Enable Automatic Assignment to Customer Group]** is ingeschakeld, is de klantenaccount niet automatisch toegewezen aan de `customerGroup` wanneer het BTW-identificatienummer van de klantenrekening is verwijderd.
