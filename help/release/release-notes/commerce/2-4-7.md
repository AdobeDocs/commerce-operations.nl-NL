...
* **RabbitMQ 3.13 steun**. Deze release is compatibel met de nieuwste versie van RabbitMQ 3.13. De compatibiliteit blijft behouden met RabbitMQ 3.11 en 3.12, die tot respectievelijk augustus 2024 en december 2024 worden ondersteund, maar Adobe beveelt aan Adobe Commerce 2.4.7 alleen te gebruiken met RabbitMQ 3.13. <!-- AC-9659 -->

* **Composer 2.7.x**. Compatibiliteit met Composer 2.2.x blijft behouden. <!-- AC-9663 -->

* **Varnish geheime voorgeheugen 7.4 steun**. Deze release is compatibel met de nieuwste versie van Varnish Cache 7.4. De compatibiliteit blijft behouden met de 6.0.x- en 7.2.x-versies, maar we raden u aan Adobe Commerce 2.4.7 alleen te gebruiken met Varnish Cache versie 7.4 of versie 6.0 LTS. <!-- AC-9662 -->
...
* De integratie van Adobe Commerce [&#x200B; &#x200B;](https://experienceleague.adobe.com/nl/docs/commerce-admin/stores-sales/delivery/shipping-carriers/fedex) met FedEx is gemigreerd van erfenis de Diensten van het Web van FedEx WSDL aan recentste RESTful APIs FedEx. FedEx Web Services Tracking, de Bevestiging van het Adres, en bevestigt WSDLS van de PostCodes zullen in Mei 2024 worden gepensioneerd. <!-- AC-1470 -->
...
* [Adobe Stock](https://experienceleague.adobe.com/nl/docs/commerce-admin/content-design/wysiwyg/adobe-stock/adobe-stock)
* [Braintree](https://experienceleague.adobe.com/docs/commerce-admin/stores-sales/payments/braintree.html?lang=nl-NL)
* [Betalingsdiensten](https://experienceleague.adobe.com/docs/commerce/payment-services/guide-overview.html?lang=nl-NL)
...
>[!NOTE]
>
>De release van 2.4.7 is compatibel met B2B 1.4.2. Adobe raadt klanten echter aan te wachten op de aanstaande B2B 1.5.0 GA-release voordat ze een upgrade uitvoeren, inclusief ondersteuning voor PHP 8.3. Zie [&#x200B; B2B versienota&#39;s &#x200B;](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html?lang=nl-NL).
...
* **3DS steun voor[!DNL Google Pay]** - Inbegrepen 3DS controlesteun voor [!DNL Google Pay] niet-aan elkaar geplaatste kaarten. Zie de [&#x200B; documentatie van Braintree &#x200B;](https://developer.paypal.com/braintree/docs/guides/3d-secure/advanced-options/javascript/v3#verify-google-pay-card-using-3ds) voor meer informatie.
...
de vraag steunt gebruiker-in werking gestelde de annuleringsverzoeken van de orde. Zie [&#x200B; Vraag de configuratie van de de ordecorrectie van een opslag &#x200B;](https://developer-stage.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/#query-a-stores-order-cancelation-configuration)
...
[&#x200B; de Opties van de Betaling &#x200B;](https://experienceleague.adobe.com/docs/commerce/payment-services/payments-checkout/payments-options.html?lang=nl-NL) introduceert gesteunde betalingsmethodes.
...
* `PUT /V1/products/attributes/{attributeCode}/is-filterable/{isFilterable}`. Padparameters: `attributeCode` (`String`) en `isFilterable` (`int` waarden zijn: 0 is nr.; 1 is `Filterable (with results)`; 2 is `Filterable (no results)`).
...
* `bin/magento setup:install` wordt nu voltooid nadat `app/etc/config.php` is verwijderd. Eerder werd het ontbrekende bestand niet opnieuw gegenereerd tijdens de installatie en heeft Commerce een fout gegenereerd. [&#x200B; GitHub-37805 &#x200B;](https://github.com/magento/magento2/issues/37805)
...
* Correctie van een fout die optreedt wanneer u het eindpunt `POST V1/shipment` gebruikt om een verzending te maken die een bundelproduct bevat. Het eindpunt voegt nu punten toe zoals verwacht en keert niet meer deze fout terug: `The shipment couldn't be saved`.
...
* Vast-korting de bedragen die op het wortelniveau worden toegepast zijn nu correct.[&#x200B; GitHub-37496 &#x200B;](https://github.com/magento/magento2/issues/37496)
...
* Adobe Commerce meldt deze fout niet meer tijdens het afrekenen wanneer persistente winkelwagentjes zijn ingeschakeld: `Invalid state change requested`. Eerder, toen deze fout werd geworpen, werden de veelvoudige orden met het zelfde `quote_id` af en toe gecreeerd.
...
* Gast-API-aanvragen voor `POST V1/guest-carts/<cartId>/shipping-information` retourneren niet langer de status `cancel` als de pagina tijdens de aanmelding in een mobiele omgeving opnieuw wordt geladen. Eerder, wierp Adobe Commerce een fout 500 en registreerde deze uitzondering: `TypeError: Argument 2 passed to Magento\CustomerCustomAttributes\Model\Plugin\ProcessCustomerShippingAddressCustomAttributes::beforeSaveAddressInformation() must be of the type string, null given`.
...
* Commerce voegt geen productbeschrijvingstekst meer in het veld productdetailpagina **[!UICONTROL Meta Description]** in als dit veld leeg is gelaten. Wanneer een productbeschrijving HTML-code bevatte die door Page Builder of een andere HTML-editor was gegenereerd, werd de HTML weergegeven in het veld Meta **[!UICONTROL Description]** en kon dit veld niet leeg laten.
...
* Veranderde de naam van de `reponseBody` variabele in `responseBody` in `app/code/Magento/CatalogRule/Controller/Adminhtml/Promo/Catalog/NewConditionHtml.php`.[&#x200B; GitHub-38093 &#x200B;](https://github.com/magento/magento2/issues/38093)
...
* De kernmodule `magento/module-release-notification` is gemarkeerd als verouderd in de 2.4-ontwikkelcode met deze opmerking: `Starting from Magento 2.4.7,  Magento_ReleaseNotification module is deprecated in favor of another in-product messaging mechanism` .
...
* De tag forbidden `@author` is uit de basis van de code verwijderd, inclusief de volgende modules: `Magento_user-variable-wee` , `Magento_Catalog` en `Magento_Customer` , `Magento_Wishlist` , `Magento_Review` `Tax` en gedurende het framework. [&#x200B; GitHub-37245 &#x200B;](https://github.com/magento/magento2/issues/37245), [&#x200B; GitHub-37246 &#x200B;](https://github.com/magento/magento2/issues/37246), [&#x200B; GitHub-37249 &#x200B;](https://github.com/magento/magento2/issues/37249), [&#x200B; GitHub-37263 &#x200B;](https://github.com/magento/magento2/issues/37263)
...
* Adobe Commerce genereert deze uitzondering op het bestandssysteem niet meer wanneer u de JavaScript/CSS-cache probeert te leegmaken: `No such file or directory`.
...
* Bedrijfsbeheerders kunnen nu bedrijfsgebruikers van de winkel toevoegen. Eerder heeft Commerce deze fout geregistreerd toen de beheerder probeerde een nieuwe gebruiker toe te voegen: `report.CRITICAL: Error: Call to a member function __toArray() on null in app/code/Magento/LoginAsCustomerLogging/Observer/LogSaveCustomerObserver.php:123`.
...
* De emulatie begint nu tijdens `send()` -aanroepen nadat de emulatie tijdens de `getInfoBlockHtml()` -aanroep is voltooid en er geen fout is geregistreerd. Eerder werd het bestand `system.log` overstroomd met deze fout: `main.ERROR: Environment emulation nesting is not allowed` . [&#x200B; GitHub-35603 &#x200B;](https://github.com/magento/magento2/issues/35603)
...
* De header X-Magento-Cache-Id is verwijderd uit GraphQL-mutatieantwoorden. Aangezien mutaties statusveranderende bewerkingen zijn (zoals het toevoegen van items aan het winkelwagentje of het plaatsen van een bestelling), zijn ze niet bedoeld om in de cache te worden opgeslagen. Het opnemen van deze koptekst was misleidend en zou tot onjuist gedrag kunnen leiden als caching werd toegepast. Deze wijziging is in overeenstemming met de beste praktijken van GraphQL en de kernbeginselen van de Adobe Commerce-architectuur.
...
* Het importeren van aandelenbronnen en klantadressen met de optie **[!UICONTROL Customers and Addresses (single file)]** is nu voltooid.
...
* Met validatiecontroles wordt het importproces nu gestopt zoals u had verwacht als er geen geldige gegevens zijn om te importeren. Commerce geeft deze fout nu weer: `There are no valid rows to import`. Voorheen werd de validatie onder deze voorwaarden doorgevoerd, maar het importproces is mislukt met dit bericht: `entity values are mixed`. [&#x200B; GitHub-32905 &#x200B;](https://github.com/magento/magento2/issues/32905)
...
* `AUTO_INCREMENT` van de `inventory_source_item` -tabel wordt niet meer verhoogd bij elke `UPDATE` -bewerking. Eerder verhoogde elke update de waarde `AUTO_INCREMENT` van deze tabel, waardoor de waarde van `AUTO_INCREMENT` uiteindelijk buiten het bereik viel wanneer een nieuwe record aan de `inventory_source_item table` werd toegevoegd. Als gevolg hiervan ontvingen beheerders de volgende fout toen ze een nieuw product probeerden te maken van Admin: `Numeric value out of range...`.
...
* U kunt nu de optie Page Builder in Beheer > **[!UICONTROL Content]** > **[!UICONTROL Pages]** gebruiken om gekopieerde tekst in alle stijlen in te voegen met het element `text` . Eerder werd in de Page Builder alleen onbewerkte tekst weergegeven en werden stijlen niet opgeslagen.
...
* Commerce verwerkt nu correct 502 fouten van PayPal door het maken van bestellingen te voorkomen wanneer deze fout optreedt. Eerder werden nog steeds bestellingen gemaakt ondanks een fout van 502 met PayPal, wat tot inconsistenties tussen Commerce-records en PayPal-rekeningen leidt.
...
* Adobe Commerce genereert niet langer een fout bij het maken van een tweede gedeeltelijke factuur voor een bestelling die door Braintree wordt betaald wanneer **[!UICONTROL Enable Vault for Card Payments]** is ingeschakeld. Eerder, toen een handelaar probeerde een tweede factuur te creëren, gaf Adobe Commerce deze fout: `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.
...
* De opdracht `bin/magento setup:perf:generate-fixtures setup/performance-toolkit/profiles/ee/small.xml` wordt nu met succes uitgevoerd wanneer meerdere keren wordt uitgevoerd. Eerder veroorzaakte het deze fout wanneer meer dan eens in werking gesteld: `Coupon with the same code already exists`.
...
* Informatieve tekst onder het `Storefront Properties` invoerveld **[!UICONTROL Stores]** van het kenmerk `Search Weight` in Admin **[!UICONTROL Attributes]** > **[!UICONTROL Product]** beschrijft nu mogelijke waarden. [&#x200B; GitHub-36646 &#x200B;](https://github.com/magento/magento2/issues/36675)
...
* Bundelproducten worden nu op de juiste wijze bijgewerkt als een winkelier deze aan zijn winkelwagentje toevoegt wanneer een planningsupdate wordt uitgevoerd. Eerder, als een bundelproduct met opties aan het karretje werd toegevoegd toen een planningtaak in proces was, had Commerce deze uitzondering: `Some of the products below do not have all the required options. Please select all required options`.
...
* De Cumulatieve waarden van de Verschuiving van de Lay-out (CLS) zijn verbeterd voor productdetailpagina&#39;s. (Dit metrisch kwantificeert de visuele stabiliteit van paginalay-outs.) [&#x200B; GitHub-35410 &#x200B;](https://github.com/magento/magento2/issues/35410)
...
* Foutberichten die zijn gekoppeld aan het verzenden van opmerkingen op de pagina met orderdetails, zijn nu informatiever. Eerder, toen een verkoopster **[!UICONTROL Submit comment]** klikte zonder of ordestatus te veranderen of tekst in het commentaargebied in te gaan, toonde Adobe Commerce dit bericht: `The comment is missing. Enter and try again` .
...
* Het `crossorigin` attribuut is toegevoegd aan `linkTypes` voor `HEAD` elementen.[&#x200B; GitHub-32509 &#x200B;](https://github.com/magento/magento2/issues/32509)
...
* Commerce genereert niet meer een fout wanneer u probeert om 301/302 te creëren die voor een product URL opnieuw richt wanneer **categorie/product URL herschrijft** wordt toegelaten. Wanneer een product wordt gecreeerd en aan een categorie wordt toegewezen, worden de geautomatiseerde herschrijvingen URL gecreeerd: één voor de directe URL en één voor het product door de categoriepad. Wanneer een nieuwe product-URL wordt herschreven, worden de correlaties tussen product, categorie en winkel gecontroleerd. Deze controle is nu gebaseerd op gecorreleerde entiteitseigenschappen (categorie en product). Eerder werd deze controle uitgevoerd op bestaande URL-herschrijvingen.
...
* Asynchrone bulksgewijs REST API-bewerkingen om producten met laagprijzen en verschillende kenmerksets bij te werken zijn nu voltooid. Eerder konden producten met verschillende kenmerksets niet op deze manier worden bijgewerkt. Deze fout werd door Commerce weergegeven: `The product was unable to be saved. Please try again`.
...
U kunt Adobe Commerce 2.4.7 installeren gebruikend [&#x200B; Composer &#x200B;](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html?lang=nl-NL).
