...
* **Native tarief beperkt voor betalingsinformatie die door REST en GraphQL APIs wordt overgebracht**. De handelaren kunnen tarief nu vormen die ](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/sales#rate-limiting) voor de betalingsinformatie beperken die REST en GraphQL wordt overgebracht. [ Deze extra laag van bescherming steunt preventie van het behandelen van aanvallen en vermindert potentieel het volume van het behandelen aanvallen die vele creditcardaantallen in één keer testen. Dit is een verandering in het standaardgedrag van een bestaand REST eindpunt. Zie [ het Beperken van het Tarief ](https://developer.adobe.com/commerce/webapi/get-started/rate-limiting/).
...
* **PHP 8.3 verenigbaarheid**. Deze release introduceert ondersteuning voor PHP 8.3. Commerce ondersteunt nu zowel PHP 8.3 als 8.2. PHP 8.2 wordt ondersteund tot de einddatum van de service (EOS) in december 2025. Na december 2025 zouden alle handelaren met 2.4.7 implementaties moeten migreren naar PHP 8.3. <!-- AC-8988 -->

Adobe Commerce 2.4.7 is nog steeds alleen voor upgradedoeleinden compatibel met PHP 8.1. PHP 8.1 wordt niet ondersteund en wordt niet aanbevolen. Adobe Commerce 2.4.7-kerncode, alle gebundelde extensies en alle Adobe-extensies en SaaS-services zijn compatibel met PHP 8.3. <!-- AC-9668 -->

* **RabbitMQ 3.13 steun**. Deze release is compatibel met de nieuwste versie van RabbitMQ 3.13. De compatibiliteit blijft behouden met RabbitMQ 3.11 en 3.12, die worden ondersteund tot respectievelijk augustus 2024 en december 2024, maar Adobe raadt aan Adobe Commerce 2.4.7 alleen te gebruiken met RabbitMQ 3.13. <!-- AC-9659 -->

* **Composer 2.7.x**. Compatibiliteit met Composer 2.2.x blijft behouden. <!-- AC-9663 -->

* **Varnish geheime voorgeheugen 7.4 steun**. Deze release is compatibel met de nieuwste versie van Varnish Cache 7.4. Compatibiliteit blijft behouden met de 6.0.x- en 7.2.x-versies, maar we raden u aan Adobe Commerce 2.4.7 alleen te gebruiken met Varnish Cache versie 7.4 of versie 6.0 LTS. <!-- AC-9662 -->
...
* Verzendmodules voor Temando zijn verwijderd uit de basis voor Commerce-code. Deze functie is vervangen in Adobe Commerce 2.4.4. <!-- AC-8130 -->
...
* De handelaars van de onderneming kunnen tot **één miljoen actieve, op coupon-Gebaseerde regels van de kartprijs** in Adobe Commerce zonder significante prestatiesdegradatie van kar en controleverrichtingen nu vormen. <!-- ACPT-1198 -->
...
* **JSON formaat nu gesteund voor de REST [ Invoer API ](https://developer.adobe.com/commerce/webapi/rest/modules/import/)**. Merchants kunnen nu maximaal 100.000 records per minuut importeren in Adobe Commerce in JSON-indeling. <!-- ACPT-943 -->
...
Het Adobe Stock-pakket (`adobe-stock-integration`) is nu verpakt in het meta-pakket voor Commerce-extensies (`extensions-metapackage` ) voor ondersteuning van frequentere updates. <!-- CABPI-630 -->
...
Het Adobe IMS-integratiepakket (`adobe-commerce/adobe-ims-metapackage`) is nu verpakt in het metapakket voor Commerce-extensies (`extensions-metapackage` ) voor ondersteuning van frequentere updates. <!-- CABPI-630 -->
...
* GraphQL-ondersteuning is toegevoegd voor kernactiviteiten voor alle betalingsmethoden behalve voor Venmo. De nieuwe eindpunten van GraphQL voor betalingsdiensten worden beschreven in [ Controle ](https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/). <!-- BUNDLE-3326 -->
...
* De betalingsworkflow bevat nu een expresbetalingsgedeelte met de knoppen PayPal, Google Pay en Apple Pay **[!UICONTROL Express]** . <!-- BUNDLE-3322 -->
...
>[!NOTE]
>
>Voor het eerst bevatten verschillende REST-eindpuntroutes de tekenreeks `V2` in plaats van `V1` . Bijvoorbeeld `GET /rest/default/V2/carts/{cartId}/coupons` . Voor Commerce-handelaren moeten deze eindpunten de `V1` -versies van de eindpunten vervangen. Deze eindpunten zijn slechts beschikbaar in Adobe Commerce.<!-- LYNX-208 -->
...
* `bin/magento setup:install` Wordt nu voltooid nadat `app/etc/config.php` het is verwijderd. Voorheen werd het ontbrekende bestand niet opnieuw gegenereerd tijdens de installatie en genereerde Commerce een foutmelding. [ GitHub-37805 ](https://github.com/magento/magento2/issues/37805)
...
* Het genereren van een databaseback-up werkt nu zoals verwacht van zowel de beheerdersregel als de opdrachtregel. Eerder gooide Adobe Commerce deze foutmelding: `Syntax error or access violation: 1064 You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'delimiter' at line 1, query was: delimiter ;;`.

<!-- ACP2E-1604 -->

* Als u wordt uitgevoerd `setup:config:set` zonder de `--lock-db-prefix` parameter op te geven, wordt de huidige waarde niet meer uit het `env.php` bestand gewist.

<!-- ACP2E-1498 -->

* De configuratie van Varnish is bijgewerkt om te voorkomen dat gastgebruikers toegang krijgen tot inhoud in de cache die verband houdt met andere klantgroepen.
...
* `bin/magento setup:upgrade` Er wordt nu een informatiever foutbericht weergegeven wanneer een onderwerp in de berichtenwachtrij geen onderwerpnaam bevat. [ GitHub-34246 ](https://github.com/magento/magento2/issues/34246)

<!-- https://github.com/magento/magento2/pull/36239 -->

* `bin/magento setup:upgrade` geeft nu een informatiever foutbericht weer wanneer samengevoegde XML-bestanden ongeldig zijn. De foutmelding bevat nu de bestandsnaam.

### Admin gebruikersinterface

<!-- AC-10998 -->

* Grafieken zijn nu standaard uitgeschakeld op het dashboard. [ GitHub-38430 ](https://github.com/magento/magento2/issues/38430)
...
* In het gedeelte Rapport van het raster Producten in winkelwagentjes wordt nu de juiste productprijs weergegeven wanneer **[!UICONTROL Catalog Price Scope]** deze is ingesteld op **[!UICONTROL Website]**. Eerder werd de productprijs weergegeven als nul toen het product werd toegewezen aan een andere website dan de standaardwebsite.
...
* De manier waarop bannerinhoud wordt geladen is geherstructureerd om buitensporige databasequery&#39;s te verwijderen, wat resulteert in verbeterde laadtijden van winkelwagentjes.

### Bundel producten

<!-- ACP2E-1464 -->

* Het aantal productcategorieën in de categorie is nu correct verantwoord voor bundelproducten wanneer **[!UICONTROL Display Out Of Stock*]** deze is ingeschakeld.
...
* De laadtijden van de afhandelingspagina zijn verbeterd voor klanten met grote adresboeken. Commerce verwerkt nu alleen het aantal klantadressen dat is opgegeven in de instelling **[!UICONTROL Customer Addresses Limit]** . Eerder, geladen Commerce het volledige adresboek.
...
* Admin-gebruikers met toegang tot websites kunnen nu afbeeldingen toevoegen aan een product dat binnen hun bereik valt. Als de beheerder geen toegangsrechten heeft voor het product, geeft Commerce een informatief bericht weer. Voorheen konden beheerders met beperkte toegang tot de website geen afbeeldingen toevoegen aan producten waartoe ze toegang hadden.

<!-- ACP2E-1956 -->

* Door de gebruiker gedefinieerde `special_from_date`, `special_to_date`, `news_from_date`, en `news_to_date` waarden worden niet overschreven wanneer een product wordt bewerkt in het winkelbereik.

<!-- AC-6761 -->

* Commerce geeft nu de juiste naam weer na het sorteren van producten op naam in Admin > **[!UICONTROL Category]** > **[!UICONTROL Products in category]** in multistore-implementaties waarbij productnamen per winkel verschillen. [ GitHub-36208 ](https://github.com/magento/magento2/issues/36208)

<!-- ACP2E-1975 -->

* De mutatie `addProductsToCart` retourneert nu aanpasbare opties met gegevens. Voorheen gaf het alleen lege aanpasbare opties op. [ GitHub-37599 ](https://github.com/magento/magento2/issues/37599)
...
* Code toegevoegd ter ondersteuning van de standaard compressieverwerking van vernis. [ GitHub-38309 ](https://github.com/magento/magento2/issues/38309)

<!-- AC-8502 -->

* De `bin/magento varnish:vcl:generate` opdracht bevat nu een `input-file argument`. Dit ondersteunt de toevoeging van een aangepaste VCL aan een Git-opslagplaats, ten opzichte van de Commerce-root. [ GitHub-37363 ](https://github.com/magento/magento2/issues/37363)

<!-- AC-10739 -->

* Ondersteuning voor Varnish 4.x en 5.x is verwijderd uit de codebase. [ GitHub-38304 ](https://github.com/magento/magento2/issues/38304)

<!-- AC-8023 -->

* `phpcodesniffer-composer-installer` In `composer.json` is geüpgraded naar v1.0.0. [GitHub-36913](https://github.com/magento/magento2/issues/36913)
...
* Geherstructureerde code in de codebase die werd gemaakt toen de coderingspraktijk dicteerde dat elke cachesectie die in het `app/etc/env.php` bestand was opgenomen, een frontend-sectie zou bevatten. Dit refactoring lost potentiële problemen op wanneer het bevorderen van de plaatsingen van Commerce pre-2.4.4. [ GitHub-38363 ](https://github.com/magento/magento2/issues/38363)
...
* De waarde voor automatisch verhogen in de `catalog_product_entity_varchar` tabel wordt nu correct verhoogd nadat een product is opgeslagen. Voorheen steeg deze waarde met tien.

<!-- ACP2E-1701 -->

* Adobe Commerce genereert deze uitzondering voor het bestandssysteem niet langer wanneer u de JavaScript-/CSS-cache probeert te legen: `No such file or directory`.

<!-- ACP2E-1358 -->

* De waarde van `value_id` in `customer_entity_int` is nu correct verhoogd. Een updatequery wordt uitgevoerd in plaats van bij het bijwerken van `insert on duplicate key update` een entiteit die deze kenmerken bevat. Als gevolg hiervan groeien de auto-increment kolommen die in het EAV-model worden gebruikt nu op een lineaire manier. Voorheen werden bij het proces voor automatisch verhogen voor kolommen waarden overgeslagen vanwege mislukte invoegquery&#39;s. [ GitHub-28387 ](https://github.com/magento/magento2/issues/28387)
...
* De stalen `Helper` zijn geherstructureerd om misleidende importaliassen te verwijderen. [ GitHub-31373 ](https://github.com/magento/magento2/issues/31373)
...
* Admin-gebruikers met beperkte toegang tot een specifieke winkel kunnen nu massale updates uitvoeren om productrevisies bij te werken. Eerder had Commerce deze uitzondering: `report.CRITICAL: TypeError: array_intersect(): Argument #1 ($array) must be of type array, null given in app/code/Magento/AdminGws/Model/Models.php:439` .
...
* De verwante productregel **is één van** voorwaarde werkt nu correct wanneer gevormd met de **Producten om** het plaatsen aan te passen. Voorheen werden in deze regel geen producten geïdentificeerd.

<!-- ACP2E-2164 -->

* Het CMS-paginaraster filtert nu de pagina&#39;s op basis van de winkelweergave die aan de beheerder is toegewezen. Voorheen werd rolbereik niet toegepast.
...
* Bedrijfsbeheerders kunnen nu bedrijfsgebruikers van de winkel toevoegen. Eerder heeft Commerce deze fout geregistreerd toen de beheerder een nieuwe gebruiker probeerde toe te voegen: `report.CRITICAL: Error: Call to a member function __toArray() on null in app/code/Magento/LoginAsCustomerLogging/Observer/LogSaveCustomerObserver.php:123` .
...
* Het gebied van het de klantenadres van Admin **Staat/van de Provincie** behoudt nu de laatste bewaarde waarde zoals verwacht. Eerder, toen u de waarde uit dit gebied verwijderde, bleef Commerce de geschrapte waarde op het **Staat/Provincie** gebied tonen. [ GitHub-36846 ](https://github.com/magento/magento2/issues/36846)

<!-- ACP2E-1796 -->

* Adobe Commerce genereert niet langer een schending van een beperking wanneer u eerst de selectie opheft en vervolgens een nieuwe CMS-pagina selecteert.
...
* De `setGiftOptionsOnCart` -mutatie verwijdert nu de terugloop van geschenken uit een volgorde zoals u had verwacht. De waarde van `gift_wrapping_id` wordt nu ongedaan gemaakt op het niveau van de winkelwagentje. Eerder werd `gift_wrapping_id` bijgewerkt naar de vorige waarde nadat de bestelling was geplaatst, zelfs toen deze waarde op het niveau van de winkelwagentje werd opgeheven.
...
* De `setGiftOptionsOnCart` -mutatie werkt de database nu correct bij wanneer u een cadeaubericht uit het winkelwagentje verwijdert. Eerder werden de berichtgegevens niet bijgewerkt in de database en werd het geschenk niet uit de winkelwagentje verwijderd.

<!-- ACP2E-1741 -->

* Het label voor het cadeauregister `Event Information` wordt nu naar behoren vertaald.
...
* U kunt nu de `updateCartItems` -mutatie gebruiken om een cadeaukaart bij te werken in een winkelwagentje. Voorheen kon door deze mutatie alleen het bedrag van de kaart in het winkelwagentje worden bijgewerkt.
...
* De query `products` retourneert nu de juiste labels wanneer er meerdere winkelweergaven zijn. [ GitHub-29635 ](https://github.com/magento/magento2/issues/29635)
...
* U kunt nu een fragment `ProductCard` gebruiken bij het opvragen van `related_products` , `upsell_products` en `crosssell_products` on `ProductInterface` . [ GitHub-29769 ](https://github.com/magento/magento2/issues/29769)
...
* Commerce houdt nu rekening met de instelling **[!UICONTROL Disable Automatic Group Change Based on VAT ID]** tijdens het importeren van klantrecords. Eerder werd de waarde `disable_auto_group_change` in de importbestanden genegeerd. [ GitHub-36409 ](https://github.com/magento/magento2/issues/36409)
...
* De brongegevens van de voorraad worden nu met succes geïmporteerd tijdens het importproces van de winkel. Commerce geeft nu een volledig meldingsbericht weer wanneer het importproces is voltooid. Eerder ontbrak het meldingsbericht.
...
* Het importeren van producten wordt niet meer onderbroken wanneer productafbeeldingen ontbreken. Eerder, toen een productbeeld tijdens de invoer van een derservice gebruikend een Csv- dossier ontbrak, toonde Adobe Commerce deze fout: `Maximum error count has been reached or a system error is occurred!`.
...
* De problemen met de weergave op de pagina met productdetails van de winkel die te wijten waren aan indexering van catalogusmachtigingen, zijn opgelost. Eerder werd de indextabel eerst door Adobe Commerce gewist en vervolgens opnieuw geïndexeerd tijdens een volledig `catalogpermissions_product` - of `catalogpermissions_category` -indexeringsproces. De catalogusmachtigingen werkten pas zoals verwacht op de storefront als het opnieuw indexeren is voltooid.
...
* Adobe Commerce neemt het volledige pad naar de hoofdmap van de server niet meer op in het foutbericht dat wordt weergegeven wanneer een onjuist geformuleerde HTTP-aanvraag naar de server is verzonden. Alleen tekenreeksen worden nu als tweede argument doorgegeven aan de methode `explode` .
...
* Handelaars kunnen kopers nu laten weten dat een bestelling gereed is voor ophalen wanneer de instelling **[!UICONTROL No Manage Stock]** is ingeschakeld voor een product. Eerder, toen deze instelling werd ingeschakeld, gaf Commerce het volgende foutbericht weer: `Your order is not ready for pickup` .

<!-- ACP2E-2227 -->

* De handelaren kunnen nu een creditnota voor orden tot stand brengen waarvan een eenvoudig product dat met een configureerbaar product wordt geassocieerd is geschrapt. Eerder konden handelaren de creditnota niet maken, en Commerce heeft een uitzondering `TypeError` gemaakt.
...
* De configureerbare status van de productvoorraad wordt nu correct bijgewerkt wanneer een configureerbaar product en de waarde van de onderliggende producten `Stock Availability` wordt bijgewerkt door een update van de massavoorraad. Voorheen kon u een configureerbaar product en de onderliggende producten niet weer op voorraad brengen door middel van een massale update.

<!-- ACP2E-2096 -->

* Het gegevenstype voor niet-standaardbronnen is bijgewerkt naar `DECIMAL (12, 4)` maximaal acht gehele getallen, wat dezelfde limiet is die is geïmplementeerd voor standaardvoorraad. Deze waarde wordt nu gesynchroniseerd met de productpagina&#39;s voor het toevoegen en bewerken van beheerders, die invoervalidatie ondersteunen voor maximaal acht cijfers voor alle typen voorraadbronnen. Voorheen werd de invoer voor het **[!UICONTROL Quantity]** veld (voor niet-standaardbronnen) slechts in maximaal zes cijfers ondersteund.

<!-- ACP2E-2054 -->

* Zowel standaard- als niet-standaardbronnen geven nu de juiste informatie weer voor de verkoopbare hoeveelheid van het product wanneer een niet-standaardbron die in de etalage is besteld, wordt verwerkt voor verzending. Eerder gaf Adobe Commerce een onjuiste voorraadstatus voor productbronnen weer.
...
* Handelaars kunnen nu afbeeldingen met dezelfde naam, maar in verschillende gevallen (zowel hoofdletters als kleine letters) uploaden naar de Medialerie. Eerder waren namen van afbeeldingsbestanden niet hoofdlettergevoelig in Page Builder en werden bestanden met dezelfde namen, behalve hoofdletters en kleine letters, overschreven in Commerce-media-opslag die werden gehost op hoofdlettergevoelige bestandssystemen.
...
* Commerce past nu het juiste kortingsbedrag toe op gedeeltelijk geannuleerde orders. Eerder werd het kortingsbedrag, zoals berekend in de kolom `base_discount_cancelled` , onjuist berekend wanneer een bestelling werd gewijzigd. Er is steun voor negatieve waarden toegevoegd aan facturen voor gedisconteerde bestellingen.

<!-- ACP2E-2585 -->

* Met het eindpunt REST `V1/orders` worden nu cadeaukaarten toegepast zoals u had verwacht bij het importeren van bestellingen. Eerder, werden de veranderingen in de attributen van de kaartextenuitbreiding in dit eindpunt niet bewaard.

<!-- ACP2E-2412 -->

* Commerce sorteert nu aangepaste adreskenmerken van de klant op basis van de sorteervolgorde die was opgegeven toen de beheerder de bestelling in Admin maakte.

<!-- ACP2E-2400 -->

* De pagina Verkooporderraster voor beheerders wordt nu op de verwachte manier geladen wanneer een handelaar een zoekopdracht vanuit deze pagina start. Eerder, toen een handelaar probeerde om orden van deze lijst te zoeken, vulde de pagina, en Commerce een `Request-URI Too Long` fout in de browser console.
...
* Adobe Commerce genereert niet langer een fout op de retourpagina van de opslagvolgorde wanneer het configureerbare product dat wordt geretourneerd, is verwijderd. Eerder, registreerde Adobe Commerce deze fout: `report.CRITICAL: Error: Call to a member function getShipmentType() on null in magento2ee/app/code/Magento/Rma/view/frontend/templates/return/create.phtml:52`.
...
* De pagina met creditnota loopt niet meer vast wanneer de optie voor gratis verzending is ingeschakeld wanneer de verzendprijs inclusief belasting is en belastingberekeningen na kortingen worden toegepast. Eerder had Adobe Commerce een uitzondering van het type `Division by zero` . [ GitHub-36800 ](https://github.com/magento/magento2/issues/36800)

<!-- AC-1342 -->

* Het gebruik van het `rest/V1/orders/{id}/comments` eindpunt om een opmerking over een bestelling te plaatsen zonder de bestelstatus op te geven, heeft geen invloed meer op de weergave van de bestelling. Eerder werd de orderstatus geregistreerd als NULL in de tabellen `sales_order` en `sales_order_grid` en de volgorde werd noch weergegeven op de pagina Mijn bestellingen, noch op het orderraster Admin. [ GitHub-34180 ](https://github.com/magento/magento2/issues/34180)
...
* De Bouwer van de pagina toont nu foutenmeldingen zoals nodig wanneer u probeert om een beeld te uploaden groter dan de gespecificeerde maximumdossiergrootte. Voorheen gaf Commerce een pop-upbericht weer voor geüploade afbeeldingsbestanden die kleiner waren dan het opgegeven maximum.

<!-- ACP2E-1992 -->

* De productschuifregelaars voor CMS-pagina&#39;s bevatten nu de bovenliggende producten van alle onderliggende producten die zijn geselecteerd voor opname in de schuifregelaar. Voorheen werden bovenliggende producten uitgesloten van de slider.
...
* U kunt nu de optie Page Builder in Beheer > **[!UICONTROL Content]** > **[!UICONTROL Pages]** gebruiken om gekopieerde tekst in alle stijlen in te voegen met het element `text` . Voorheen werd in Page Builder alleen tekst zonder opmaak weergegeven en werden geen stijlen opgeslagen.

<!-- ACP2E-1646 -->

* CORS-fouten verschijnen niet meer tijdens het bewerken van productinhoud met Page Builder wanneer de admin-gebruiker een specifieke ACL heeft. De beheerder kan het product nu opslaan. Voorheen sloeg Page Builder het product niet op en liep het opslagproces vast.

<!-- ACP2E-1852 -->

* Nauwkeurige informatie over de bestandsgrootte is nu opgenomen in het afbeeldingsvoorbeeld voor geüploade afbeeldingen in Page Builder.

<!-- ACP2E-2016 -->

* Page Builder-pagina-elementen worden nu met een acceptabele snelheid weergegeven in de Chrome-browser (v112). Voorheen was het opslaan van inhoud met Page Builder in de Chrome-browser traag en Commerce registreerde deze fout in de browserconsole: `[ERROR] Page Builder was rendering for 5 seconds without releasing locks`.
...
* Als u een pagina met Admin-productbewerkingen laadt, wordt niet langer een periodiek `Uncaught TypeError` JavaScript-fout gegenereerd.

<!-- ACP2E-1886 -->

* Afbeeldingen die zijn geüpload via de Page Builder-editor, worden nu gecodeerd als gebruikt in **[!UICONTROL Content]** > **[!UICONTROL Media Gallery]** . Eerder, werden deze beelden duidelijk als **niet overal** in de Galerij van Media gebruikt.
...
* Commerce verwerkt nu correct 502-fouten van PayPal door te voorkomen dat er bestellingen worden gemaakt wanneer deze fout optreedt. Voorheen werden bestellingen nog steeds aangemaakt ondanks een 502-fout van PayPal, wat leidde tot inconsistenties tussen handelsrecords en die van PayPal.

<!-- AC-9142 -->

* PayPal Express Checkout-transacties omvatten nu SKU&#39;s van onderliggende producten en volledige producttitels. Het `Magento/Paypal/Model/Cart.php` bestand verzamelt nu onderliggende product-SKU&#39;s met hun bovenliggende producttitel om de producttitel te identificeren met de onderliggende SKU. [ GitHub-36808 ](https://github.com/magento/magento2/issues/36808)

<!-- ACP2E-1964 -->

* De **[!UICONTROL Card Security Code Does Not Match]** configuratie-instelling wordt nu verwerkt zoals verwacht tijdens het Payflow Pro-betalingsproces. Voorheen werd dit evaluatieproces nooit voltooid, waardoor het afrekenen werd geblokkeerd.
...
* Problemen met de verwerking van PayPal en PayPal-tegoedbetaalmethoden zijn opgelost.
...
* Gedeeltelijk gefactureerde bestellingen worden nu zoals verwacht vastgelegd in het juiste account in implementaties waarbij winkeliers twee verschillende verkopersaccount-ID&#39;s voor dezelfde valuta in hun Braintree-account hebben.

<!-- BUNDLE-3250 -->

* Adobe Commerce genereert niet langer een fout bij het maken van een tweede gedeeltelijke factuur voor een bestelling die door Braintree wordt betaald wanneer **[!UICONTROL Enable Vault for Card Payments]** is ingeschakeld. Eerder, toen een handelaar probeerde om een tweede factuur te creëren, gaf Adobe Commerce deze fout: `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.
...
* Klanten kunnen nu afrekenen met Braintree Payments en Adobe Commerce 2.4.6 met onzichtbare ReCAPTCHA v2 of v3. [ GitHub-37241 ](https://github.com/magento/magento2/issues/37241)
...
* De prestaties van de mutatie `addProductToCart` bij uitvoering met veel aangepaste productkenmerken zijn verbeterd. Eerder, toen deze mutatie werd uitgevoerd met veel productkenmerken (ongeveer 1000), probeerde Commerce alle productkenmerken te laden.
...
* GraphQL-transactienamen worden nu correct weergegeven nadat New Relic is uitgeschakeld in Commerce Admin. [ GitHub-36112 ](https://github.com/magento/magento2/issues/36112)
...
* Retourpunten kunnen nu worden terugbetaald na het aanmaken van een creditnota. Eerder konden handelaren geen beloningspunten terugbetalen nadat ze een creditmemo hadden gemaakt en bleef de orderstatus `Completed` behouden, niet `Closed` .

### RMA

<!-- ACP2E-2246 -->

* Met de REST-aanvraag `POST /V1/returns` wordt nu één RMA per product gemaakt. Voorheen kon dit verzoek de validatie omzeilen en meerdere RMA&#39;s per product retourneren.

<!-- ACP2E-2450 -->

* De attributen RMA worden nu automatisch toegevoegd aan de standaardEAV vorm wanneer het **[!UICONTROL Show on Storefront]** attribuut RMA aan **geen** wordt geplaatst. Eerder, was pop-up van de het puntdetails RMA leeg toen een attribuut van de douanerestitutie werd gecreeerd en het **[!UICONTROL Show on Storefront]** attribuut RMA werd geplaatst aan **nr**.

<!-- ACP2E-2211 -->

* De waarde van het kenmerk **[!UICONTROL Enable RMA]** komt nu overeen met de waarde die is opgegeven in de configuratie wanneer het selectievakje **[!UICONTROL Use Config]** op de productbewerkingspagina is ingeschakeld.

<!-- ACP2E-1230 -->

* Het attribuut **[!UICONTROL Enable RMA]** op het individuele productniveau eerbiedigt nu de montages van opslagRMA configuratie. Eerder werd deze instelling standaard uitgeschakeld toen u een product toevoegde, ongeacht de configuratie-instellingen van de winkel.
...
* Etalagefilters werken nu zoals verwacht bij het sorteren van catalogusitems met aangepaste vervolgkeuzekenmerken.

<!-- ACP2E-1499 -->

* Zoekresultaten geven nu relevante resultaten weer op basis van het zoekgewicht van de kenmerken, zoals verwacht.

<!-- AC-7505 -->

* Informatieve tekst onder het invoerveld van `Storefront Properties` het `Search Weight` kenmerk op Admin **[!UICONTROL Stores]** > > **[!UICONTROL Attributes]** **[!UICONTROL Product]** beschrijft nu mogelijke waarden. [ GitHub-36646 ](https://github.com/magento/magento2/issues/36675)

### Scheepvaart

<!-- ACP2E-2515 -->

* Prijsregels voor winkelwagentjes werken nu zoals verwacht wanneer een bundelproduct aan de winkelwagen wordt toegevoegd wanneer dynamische prijskenmerken zijn uitgeschakeld. Gratis verzending en de juiste tabelsnelheid worden toegepast, ongeacht of het kenmerk dynamic is in- of uitgeschakeld. Eerder werd gratis verzending niet toegepast zoals was verwacht.

<!-- AC-9729 -->

* Commerce gaat nu over naar een nieuwe verzendpagina zoals u had verwacht na het maken van een gedeeltelijke verzending voor een bestelling. Eerder heeft Commerce na het maken van een gedeeltelijke verzending geen volledige verzendpagina gemaakt voor de resterende producten van de bestelling. [ GitHub-38063 ](https://github.com/magento/magento2/issues/38063)
...
* Commerce genereert niet langer een uitzondering tijdens het afrekenen met meerdere verzendadressen wanneer de beheerder zich heeft aangemeld als functie van klant.
...
* Bundelproducten worden nu op de juiste wijze bijgewerkt als een winkelier deze aan zijn winkelwagentje toevoegt wanneer een planningsupdate wordt uitgevoerd. Eerder, als een bundelproduct met opties aan het karretje werd toegevoegd toen een planningtaak in proces was, gaf Commerce deze uitzondering: `Some of the products below do not have all the required options. Please select all required options`.
...
* Wanneer u een geplande update bijwerkt, wordt de vorige versie van het gefaseerde item niet meer verwijderd als er geen einddatum is opgegeven. Eerder, toen een gefaseerd punt werd bijgewerkt, werd de rollen-gesteunde versie van het punt verwijderd als geen einddatum werd gespecificeerd. Commerce heeft geen fout gegenereerd of heeft beheergebruikers gewaarschuwd voor verwijdering.
...
* Er treden geen JavaScript-fouten meer op wanneer u geplande updates voor categorieën toevoegt of bewerkt wanneer Google Analytics Content Experiments is ingeschakeld.
...
* De consumentenwachtrij van `staging.synchronize_entity_period` in de `magento_bulk` -tabel wordt niet langer groter wanneer er geen testupdates voor de verwerking zijn. Eerder, toen `cron` werd gevormd om elke minuut in werking te stellen, werd een nieuw verslag met betrekking tot de `staging.synchronize_entity_period` consument toegevoegd aan deze lijst elke minuut.
...
* Commerce geeft nu de juiste waarde voor de **[!UICONTROL Regular Price]** -waarde weer wanneer een aangepaste optie wordt geselecteerd voor een product met een **[!UICONTROL Special Price]** -waarde wanneer **[!UICONTROL Price Display Settings]** is ingesteld op inclusief btw.
...
* Integratietests voor de functionaliteit van het cadeauregister bewerken zijn niet langer afhankelijk van bestaande id&#39;s van entiteiten. Alle vereiste gegevens worden nu verkregen via correcties. Eerder is `\Magento\GiftRegistry\Controller\Index\EditPostTest` mislukt.
...
* Aankondiging van vervangen voor `StorefrontDeleteItemFromRequisitionListPrintView` is gecorrigeerd.
...
* Gecorrigeerd cursorgedrag in het tekstveld voor het cadeauregister **[!UICONTROL Message]** .
...
* Verbeterde berichten over fouten die optreden tijdens het renderen van sjablonen. Commerce geeft nu een informatief bericht weer. Eerder gaf Commerce HTML-lekken weer toen er een PHP-fout optrad.
...
* Gebruikers met beheerdersrechten kunnen nu het winkellogo wijzigen als de modus Eén winkel is ingeschakeld. Eerder gaf Commerce het standaardfallback-logo weer. [ GitHub-35952 ](https://github.com/magento/magento2/issues/35952)
...
* Adobe Commerce genereert niet langer een fout wanneer u de weergave snel probeert te wijzigen tussen de geschiedenis van opmerkingen en Retourneert op de bestelpagina Admin. Eerder gaf Adobe Commerce de volgende algemene fout weer: `A technical problem with the server created an error` .
...
* Foutberichten die zijn gekoppeld aan het verzenden van opmerkingen op de pagina met orderdetails, zijn nu informatiever. Eerder, toen een winkelier **[!UICONTROL Submit comment]** klikte zonder of ordestatus te veranderen of tekst in het commentaargebied in te gaan, toonde Adobe Commerce dit bericht: `The comment is missing. Enter and try again`.
...
* Commerce genereert niet langer product-URL-herschrijvingen voor hoofdcategorieën en voor producten die zijn ingesteld op **[!UICONTROL Not Visible Individually]** in instellingen voor meerdere winkels wanneer ontbrekende URL-herschrijvingen voor aangepaste winkels worden gegenereerd.
...
* Nieuw gemaakte CMS-pagina&#39;s met één onderliggend knooppunt worden nu opgenomen in het raster voor marketingfilters (**[!UICONTROL Marketing]** > **[!UICONTROL URL rewrites]** ). Eerder werden URL-herboekingen niet gemaakt voor deze CMS-pagina&#39;s.
...
* U kunt nu het attribuut `Categories` voor Visual Merchandiser-regels gebruiken. Eerder had Adobe Commerce deze uitzondering: `Something went wrong while saving the category` .

### Web API-framework

<!-- AC-6980 -->

* Commerce geeft niet langer een `Integration not secure` waarschuwing weer in Beheer **[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** wanneer de velden **[!UICONTROL Callback URL]** en **[!UICONTROL Identity link URL]** leeg zijn. [ GitHub-36407 ](https://github.com/magento/magento2/issues/36407)
...
* U kunt nu een bestelling openen vanuit de beheerfunctie met de status `Received` als `AsyncOrder` niet is ingeschakeld. Eerder kon u de pagina met gegevens over de volgorde niet openen vanuit de beheerder toen de status van de order werd gewijzigd door `POST V1/orders` .

<!-- ACP2E-1526 -->

* Adobe Commerce retourneert nu het `content` -kenmerk (base64-afbeeldingscode) in de `GET V1/products/sku/media` -API-reacties voor productmedia. Het kenmerk `content` is niet eerder in de reactie opgenomen.

### Wissenlijst

<!-- ACP2E-1784 -->

* Commerce geeft nu een bericht weer waarin wordt aangegeven dat een item al aan een wenslijst is toegevoegd wanneer een winkelier hetzelfde item meerdere keren aan een wenslijst probeert toe te voegen. Voorheen verhoogde Commerce onder deze omstandigheden de hoeveelheid van het product in de verlanglijst.

<!-- AC-8610 -->

* Adobe Commerce voegt nu met succes producten met de vereiste geüploade aanpasbare optiebestanden toe aan de verlanglijst. Voorheen heeft Commerce twee producten aan de verlanglijst toegevoegd, niet één. [ GitHub-37437 ](https://github.com/magento/magento2/issues/37437)
...
U kunt Adobe Commerce 2.4.7 installeren gebruikend [ Composer ](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
