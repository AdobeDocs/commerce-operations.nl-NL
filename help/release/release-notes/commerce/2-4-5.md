...
* `Braintree_disabled_partial_capture_for_applepay_googlepay.zip`. Deze patch verhelpt het onvermogen van gebruikers om gedeeltelijke facturen te maken wanneer ze Apple Pay of Google Pay through Braintree als betalingsmethode gebruiken. Zie de [Adobe Commerce 2.4.4: Kan geen gedeeltelijke facturen maken](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) Kennisbank, artikel.
...
* **Adobe Commerce-implementaties ter plaatse**: Adobe Commerce 2.4.5 is getest en bevestigd dat het compatibel is met Elasticsearch 7.17 (~7.17.0 met beperkingen). Handelaren die Adobe Commerce in het bedrijf hosten, kunnen Elasticsearch of OpenSearch 1.2 gebruiken. <!-- AC-2425-->
...
* UCT identificeert nu codeprecties en geeft specifieke aanbevelingen voor het oplossen van elk probleem.  <!-- AC-3147-->
...
* Productlijsten worden nu correct weergegeven in de Admin. Eerder werd de productlijst niet gerenderd, en Adobe Commerce toonde deze fout:  `Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "<ID>" already exists`. [GitHub-33145](https://github.com/magento/magento2/issues/33145)
...
* Adobe Commerce geeft nu de vooraf gedefinieerde EAV-systeemkenmerken correct weer volgens de website-instelling op de storefront. Eerder werden klantkenmerken op websiteniveau die waren ingeschakeld voor de ene website en uitgeschakeld voor de andere, weergegeven als ingeschakeld voor beide websites.
...
* Adobe Commerce retourneert nu een fout van 404 in plaats van een fout van 500 wanneer u navigeert naar `/checkout/sidebar/updateItemQty/?item_qty=error` in de winkel. Deze fout is eerder gegenereerd:  `Warning: A non-numeric value encountered in /vendor/magento/module-checkout/Controller/Sidebar/UpdateItemQty.php on line 69`. [GitHub-34380](https://github.com/magento/magento2/issues/34380)
...
* Adobe Commerce-sessies eindigen niet meer nadat een GraphQL-aanvraag is ingediend. Eerder `ClearCustomerSessionAfterRequest` de insteekmodule heeft zich afgemeld bij de winkelier. [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
* Adobe Commerce geeft nu de gratis verzendkosten (0) weer op de totalen van de pagina Admin en de winkelfactuuradres. Eerder, toen de verzending nul was voor een bestelling, gaf Adobe Commerce het totaalbedrag aan verzendkosten niet weer op de factuurpagina in totaal.
...
* Handelaren kunnen nu een gedeeltelijke terugbetaling verzenden voor bestellingen die met Apple Pay via Braintree zijn betaald. Dit was een bekend probleem in Adobe Commerce 2.4.4.
...
* Elasticsearch vragen werken nu zoals verwacht `int` is geconfigureerd als doorzoekbare back-end `type` kenmerk. Voorheen gooide Adobe Commerce een `Elasticsearch\Common\Exceptions\BadRequest400Exception` uitzondering.
...
* Adobe Commerce geeft nu een nauwkeurige suggestie voor zoekresultaten weer, waarbij u op de winkel kunt rekenen in implementaties waar Suggesties zoeken en de **Aantal resultaten tonen voor elke suggestie** instellen is ingeschakeld. Eerder was het aantal dat naast de trefwoorden werd weergegeven nul.
