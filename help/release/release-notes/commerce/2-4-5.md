...
* `Braintree_disabled_partial_capture_for_applepay_googlepay.zip`. Deze patch verhelpt het onvermogen van gebruikers om gedeeltelijke facturen te maken wanneer ze Apple Pay of Google Pay through Braintree als betalingsmethode gebruiken. Zie [ Adobe Commerce 2.4.4: Kan geen gedeeltelijke facturen maken ](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) Knowledge Base-artikel.
...
* **Adobe Commerce op-gebouw plaatsingen**: Adobe Commerce 2.4.5 is getest en bevestigd dat het compatibel is met Elasticsearch 7.17 (~7.17.0 met beperkingen). Handelaren die Adobe Commerce in het bedrijf hosten, kunnen Elasticsearch of OpenSearch 1.2 gebruiken. <!-- AC-2425-->

* **Adobe Commerce wolk-ontvangen plaatsingen**: Elasticsearch 7.11 of hoger wordt niet ondersteund op Adobe Commerce 2.4.5 cloudgehost implementaties. OpenSearch is de standaard zoekmachine voor Adobe Commerce 2.4.5-cloudimplementaties.
...
* Adobe Commerce werpt niet meer een fout wanneer u de **Controle hier activeert om een voer van RSS aan uw 1} checkbox van de Lijst van de Weigering te verbinden alvorens op** te klikken Weigeren van het Aandeel **.**[ GitHub-34998 ](https://github.com/magento/magento2/issues/34998)
...
**Uitgave**: [!DNL RabbitMQ] Infrastructuurelementen (uitwisselingen, wachtrijen) kunnen niet worden geïnitialiseerd na een nieuwe installatie (geen upgrade) van Adobe Commerce 2.4.5. Deze onjuiste configuratie van [!DNL RabbitMQ] kan leiden tot een fatale fout wanneer asynchrone bewerkingen worden uitgevoerd of een prestatieprofiel wordt gegenereerd. Dit heeft geen invloed op cloudimplementaties. **Oplossing**: Handelaars die een nieuwe installatie van Adobe Commerce 2.4.5 op locatie uitvoeren, moeten `bin/magento setup:upgrade --keep-generated` uitvoeren nadat ze `bin/magento setup:config:set` hebben gebruikt om Adobe Commerce te configureren [!DNL RabbitMQ] of installeren door [!DNL RabbitMQ] -configuratieopties door te geven aan de opdracht `setup:install` . Zie [[!DNL RabbitMQ]](../../../installation/prerequisites/rabbitmq.md) .
