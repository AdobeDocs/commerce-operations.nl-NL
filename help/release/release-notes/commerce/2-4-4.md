...
Deze release bevat één beveiligingsoplossing en verbeteringen in de platformbeveiliging. Deze beveiligingsoplossing is gebackport naar Adobe Commerce 2.4.3-p2 en Adobe Commerce 2.3.7-p3.

Tot op heden hebben zich geen bevestigde aanvallen met betrekking tot deze problemen voorgedaan. Bepaalde kwetsbaarheden kunnen echter mogelijk worden misbruikt om toegang te krijgen tot klantinformatie of om beheerderssessies over te nemen. De meeste van deze problemen vereisen dat een aanvaller eerst toegang verkrijgt tot de beheerder. Daarom herinneren we u eraan alle noodzakelijke stappen te nemen om uw beheerder te beschermen, inclusief maar niet beperkt tot deze inspanningen:
...
* Het gebruik van e-mailvariabelen werd in 2.3.4 afgeschaft als onderdeel van een beperking van het beveiligingsrisico ten gunste van een striktere variabelesyntaxis. Dit verouderde gedrag is in deze release volledig verwijderd als voortzetting van die beperking van het beveiligingsrisico.
...
* OAuth-toegangstokens en tokens voor het opnieuw instellen van wachtwoorden zijn nu versleuteld wanneer ze in de database worden opgeslagen. <!-- AC-520 1323-->

* De validatie is versterkt om te voorkomen dat niet-alfanumerieke bestandsextensies worden geüpload. <!-- AC-479-->

* Ondersteuning voor reCAPTCHA toegevoegd aan couponcodes. <!-- AC-461-->

* De wagen is nu standaard uitgeschakeld wanneer Adobe Commerce in de productiemodus staat. <!-- AC-1450-->

* HTTPS is nu standaard ingeschakeld voor de Adobe Commerce storefront. De **instellingen Beveiligde URL&#39;s gebruiken in de etalage** en **Beveiligde URL&#39;s gebruiken in Admin** zijn standaard ingeschakeld en alle ingebouwde cookies zijn nu ingesteld als veilig.  <!-- AC-1173-->

* De plug-in voor afhankelijkheidsverwarring is nu vereist voor alle Adobe Commerce-installaties. Voorheen was deze plug-in alleen vereist voor installaties op basis van Composer. Het staat nu vertrouwde versies toe. Verkopers kunnen de beperkingen omzeilen die in deze plug-in zijn ingesteld en die bepaalde combinaties van Composer-installaties verbieden. De beperkingen kunnen worden omzeild voor vertrouwde versies en Adobe Commerce geeft nu een waarschuwing weer voordat u doorgaat met de installatie.  <!-- AC-501 970-->

* Ontwikkelaars kunnen nu de limiet configureren voor de grootte van arrays die door Adobe Commerce RESTful-eindpunten worden geaccepteerd per eindpunt. Zie [API-beveiliging](https://developer.adobe.com/commerce/webapi/get-started/api-security/). <!-- AC-465-->
...
* Terugval op gegevens voor widgetnamen. U moet de volledige naam voor de `.data()` -toets gebruiken.

* Harde codering van klassen zoals `ui-corner-all` in widgets.

### Verbeterde prestaties en schaalbaarheid

Verbeterde Adobe Commerce-prestaties verhogen de optimalisatie van verwerking van doorvoerorders en de wachtrij van berichten. De functie voor asynchrone bestellingen die in deze release wordt geïntroduceerd, ondersteunt het maken van ongeveer 60.000 bestellingen/uur. Eerdere versies van Adobe Commerce ondersteunden de verwerking van ongeveer 10.000 bestellingen/uur, wat een mogelijk knelpunt voor flash-verkoop vormde. De nieuwe functie voor meerdere consumenten ondersteunt het schalen van het aantal gebruikers in de berichtenwachtrij op één cloudinstantie en verhoogt het aantal bestellingen dat per uur wordt verwerkt.

Prestatieverbeteringen in deze release:

* De AsyncOrder-functie ondersteunt het plaatsen van snellere orders dan synchrone uitvoering. Wanneer AsyncOrder is ingeschakeld, wordt het plaatsen van bestellingen op de achtergrond uitgevoerd terwijl shoppers andere taken in de winkel uitvoeren.

* De nieuwe **configuratieoptie Voorraadcontrole inschakelen bij laden van winkelwagen** biedt een schakelbare voorraadcontrole bij het laden van een product in de winkelwagen. Het is standaard ingeschakeld. Als u deze optie uitschakelt, slaat Adobe Commerce de voorraadcontrole over terwijl de offerte wordt geladen. Hierdoor wordt het afrekenen versneld, vooral voor winkelwagentjes met veel items.
...
Deze release bevat meerdere bugfixes. Zie [B2B Release Notes](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).

### Extensies gebundeld door de leverancier

Met uitzondering van [Braintree](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree) zijn alle door de leverancier gebundelde extensies verwijderd uit de Adobe Commerce 2.4.4-codebasis. De handelaren zouden aan de officiële uitbreidingen moeten migreren, die op [ Commerce Marketplace ](https://marketplace.magento.com/extensions.html) beschikbaar zijn. <!-- AC-1165-->
...
* Winkeliers kunnen nu alternatieve tekst (`alt_text`) toevoegen aan afbeeldingen (afbeelding, banner, dia) om de toegankelijkheid van de inhoud te verbeteren. [ GitHub-746 ](https://github.com/magento/magento2-page-builder/issues/746) <!-- PB-1193-->
...
* De `bin/magento app:config:import` opdracht wordt nu foutloos uitgevoerd wanneer u waarden bijwerkt `time` in `config.php`. Voorheen resulteerde het wijzigen van de configuratie van productwaarschuwingen door handmatige bewerking (dat wil zeggen, niet met behulp van de beheerder) in een fout.
...
* Geavanceerde prijskortingen voor speciale prijzen worden nu correct weergegeven voor bundelproducten met het voorvoegsel % in het productraster. Voorheen werd een % korting ten onrechte weergegeven met het voorvoegsel $ in plaats van % in het productraster.
...
* Paginacache groeit niet langer snel bij normaal gebruik. [ GitHub-9458 ](https://github.com/magento/magento2/issues/9458)

<!-- magento/magento2/pull/33468-->

* Het plaatsen van een bestelling heeft niet langer tot gevolg dat alle cache-tags die betrekking hebben op de bestelde producten uit de Varnish-cache worden verwijderd. [ GitHub-30128 ](https://github.com/magento/magento2/issues/30128)
...
* Adobe Commerce genereert niet langer een foutmelding wanneer u de `addConfigurableProductsToCart` mutatie gebruikt om een uitgeschakeld kind van een configureerbaar product aan de winkelwagen toe te voegen. Eerder genereerde Adobe Commerce een fout in schending van integriteitsbeperkingen.

<!-- ACP2E-206-->

* Cross-selling-producten zijn niet meer zichtbaar op de winkelwagenpagina wanneer de **instelling Cross-sell-items weergeven in de configuratie van het winkelwagentje** is uitgeschakeld.

<!-- ACP2E-157-->

* De gebruikswaarde van de kortingsbon verandert niet meer als de order waarop de kortingsbon is toegepast, mislukt.  Voorheen werden automatisch gegenereerde kortingsbonnen voor eenmalig gebruik als ongeldig gemarkeerd nadat de kortingsbon in de winkelwagen was toegepast, maar de bestelling niet was geplaatst.

<!-- ACP2E-137-->

* Validatie is toegevoegd aan de checkout-workflow om het adres van de klant te vergelijken met de standaardwaarde. Als het validatieproces wijzigingen identificeert, wordt `customer_address_id` ingesteld op `null` en wordt dit behandeld als een nieuw klantadres. Eerder, als de klant het adres zonder bevestiging bij kassa opsloeg en de zelfde klant probeerde om een orde te plaatsen gebruikend het zelfde adres nadat de bevestiging werd toegepast op checkout gebied, aanvaardde Adobe Commerce niet het uitgegeven adres en wierp een uitzondering.
...
* Als u een onderliggend product bijwerkt, wordt de catalogusregelkorting die u tijdens het gedeeltelijke indexeringsproces hebt geërfd, niet meer verwijderd van het configureerbare bovenliggende product. Wanneer we voorheen een configureerbaar product toewezen aan een categorie, maar niet aan de bijbehorende eenvoudige producten, en die categorie vervolgens gebruikten om een catalogusregel te maken, verwijderde Adobe Commerce de catalogusregel die was toegewezen aan het eenvoudige product tijdens het gedeeltelijke indexeringsproces.

<!-- ACP2E-216-->

* De lijst met gegroepeerde en configureerbare producten in de rasterweergave bevat nu alleen eenvoudige en virtuele producten bij het toevoegen van productopties. Voorheen bevatte de applicatie eenvoudige producten met opties.

### Catalogus regel

<!-- MC-41807-->

* Dagelijkse updates van catalogusregels leiden niet langer tot prestatievermindering. Voorheen werden bij gedeeltelijke prijsindexering van catalogusproducten oude prijzen of datums niet verwijderd.

<!-- MC-38263-->

* Adobe Commerce verwijdert niet langer catalogusprijsregels voor configureerbare producten tijdens gedeeltelijke herindexering. Voorheen werden alleen subproducten opnieuw geïndexeerd wanneer configureerbare producten opnieuw werden geïndexeerd.

<!-- ACP2E-331-->

* Catalogusprijsregels met de **categorie is niet category_id** voorwaarde zijn nu van toepassing op producten die niet tot een categorie behoren.

### Configureerbare producten

<!-- MC-42188-->

* De configureerbare labelnaam van de productoptie is nu gebaseerd op de geselecteerde winkelweergave tijdens het maken van een beheerdersbestelling. Voorheen gebruikte Adobe Commerce de labelnaam uit de standaardwinkelweergave.
...
* Adobe Commerce genereert niet langer een interne fout wanneer u een uitgeschakeld kind van een configureerbaar product aan de winkelwagen probeert toe te voegen.

### Klant

<!-- MC-24204-->

* Adobe Commerce bekijkt nu het bereik van websites voor Admin-landinstellingen tijdens het maken van bestellingen. Voorheen werkten ordergegevens, zoals klantadreskenmerken, correct op één website, alleen in een implementatie op meerdere locaties. [ GitHub-23254 ](https://github.com/magento/magento2/issues/23254)
...
* Waarden voor regio- en landinstellings-id worden nu correct geconverteerd in e-mailsjablonen. Vroeger, wanneer u op **de voorbeeldlink** van de beheerders-e-mailsjabloon klikte, werden het land en de regionaam niet correct weergegeven.

<!-- MC-36262-->

* E-mails met verkoopupdates die door de beheerder zijn verzonden voor niet-standaard winkelweergaven, bevatten nu de juiste labels voor de orderstatus. Voorheen werd in deze e-mails altijd de status van de standaardwinkel weergegeven. [ GitHub-29263 ](https://github.com/magento/magento2/issues/29263)

<!-- AC-721-->

* Adobe Commerce bevat `Content-Disposition: inline` nu zoals verwacht kopteksten in e-mail. [ GitHub-29258 ](https://github.com/magento/magento2/issues/29258)

<!-- magento/magento2/pull/32720-->

* E-mails met order- en verzendmeldingen werken nu zoals verwacht in implementaties met Microsoft Outlook en MS Exchange Server. Voorheen was de hoofdtekst van de e-mail leeg, maar bevatte deze een bijlage met het ATT*-label. [ GitHub-25076 ](https://github.com/magento/magento2/issues/25076)
...
* De les `BundleDiscountPrice` in `magento2/app/code/Magento/Bundle/Pricing/Price/ConfiguredPrice.php` wordt nu gedeclareerd zoals verwacht. Eerder gooide Adobe Commerce deze foutmelding: `Class Magento\Bundle\Pricing\Price\BundleDiscountPrice not found`. [ GitHub-33334 ](https://github.com/magento/magento2/issues/33334)

<!-- AC-1239-->

* Escaped CSV-veldwaarden die door de AWS S3-adapter zijn geschreven, komen nu overeen met de gegevens die zijn geschreven door andere bestandssysteemadapters.
...
* Inline bewerkingen voor het herschrijven van URL&#39;s worden nu geregistreerd in rapporten.
...
* GraphQL biedt New Relic nu beschrijvende transactienamen, wat handig kan zijn bij het opsporen van fouten. [ GitHub-30915 ](https://github.com/magento/magento2/issues/30915)

<!-- MC-42903-->

* De `setShippingAddressesOnCart` mutatie ondersteunt nu het instellen van verzendadressen op een winkelwagentje met een lege telefoonnummerwaarde wanneer de **configuratie-instelling Telefoonbeheerder** weergeven is ingesteld op optioneel. Eerder gooide Adobe Commerce deze foutmelding: `Field CartAddressInput.telephone of required type String! was not provided`.

<!-- MC-42970-->

* Er is een probleem opgelost waarbij u door de `addSimpleProductsToCart` mutatie eenvoudige producten aan een winkelwagentje kon toevoegen die niet waren toegewezen aan de doelwebsite.

<!-- MC-42600-->

* MySQL-query&#39;s zijn geoptimaliseerd voor GraphQL-query&#39;s `products` die zoekparameters gebruiken om zoekresultaten te verfijnen.

<!-- MC-43189-->

* De `addConfigurableProductToCart` mutatie biedt geen ondersteuning meer voor het toevoegen van een product aan een winkelwagentje als het product niet is opgenomen in de gevraagde scope.
...
* Wanneer de prijs van een laagproduct wordt bijgewerkt vanuit de backend, wordt de nieuwe prijs correct bijgewerkt op de winkelwagen van de klant. Eerder werd, hoewel het subtotaal in de overzichtssectie van het winkelwagentje correct werd bijgewerkt, rij subtotaal niet bijgewerkt.

<!-- AC-697-->

* De `addProductsToCart` mutatie voegt nu alle geselecteerde opties toe aan de winkelwagen wanneer deze wordt gebruikt om bundelproducten met meerdere selectieopties toe te voegen. Voorheen voegde de mutatie alleen de laatste selectie toe. [ GitHub-33123 ](https://github.com/magento/magento2/issues/33123)

<!-- MC-38815-->

* GraphQL-query&#39;s retourneren nu het factuuradres zoals verwacht wanneer de waarde van een optioneel telefoonveld is ingesteld op een lege tekenreeks. Voorheen retourneerden query&#39;s een null-adreswaarde. [ GitHub-30218 ](https://github.com/magento/magento2/issues/30218)

<!-- ACP2E-51-->

* Het `products` queryantwoord geeft nu producten weer op basis van de opgegeven sorteervolgorde wanneer meerdere producten dezelfde relevantie of prijswaarde hebben.

<!-- ACP2E-75-->

* Gelijktijdige `addSimpleProductsToCart` aanvragen en `addProductsToCart` aanvragen resulteren niet langer in dubbele prijsopgave-items.

<!-- ACP2E-76-->

* `products` Gelaagde navigatiefilters voor query&#39;s retourneren nu de juiste lijsten met onderliggende categorieën. Alleen directe subcategorieën worden nu opgenomen wanneer de `includeDirectChildrenOnly` vlag in de aanvraag wordt opgegeven. Voorheen werden alle categorieën opgenomen als reactie in de aggregatiesectie wanneer een product aan verschillende categorieën was toegewezen.

<!-- ACP2E-132-->

* Zoeksuggestiefunctionaliteit toegevoegd voor GraphQL-zoekopdrachten.

<!-- ACP2E-288-->

* Het `OrderAddress.telephone` veld is nu optioneel in de `customer` query of andere query&#39;s en mutaties die orderadressen van klanten retourneren of wijzigen.

<!-- ACP2E-278-->

* Prijzen worden nu correct bijgewerkt voor klanten in een opgegeven groep in zowel de etalage als de beheerder wanneer een groeps-ID wordt gewijzigd via API. Eerder werden deze prijzen niet correct bijgewerkt wanneer een groep-id via de API werd gewijzigd.
...
* Adobe Commerce genereert niet langer een foutmelding wanneer u de afbeeldingencache leegmaakt en tegelijkertijd afbeeldingen in de etalage laadt.
...
* `lib/internal/Magento/Framework/Filter/Money.php` is afgeschaft. Het bevat de `money_format()` functie, die is verwijderd in PHP 8.x. [GitHub-33870](https://github.com/magento/magento2/issues/33870)<!-- magento/magento2/pull/34366)-->
...
* Casten naar `int` is verwijderd voor automatisch verhoogde factuur-ID&#39;s wanneer deze worden voorafgegaan door een tekenreeks. Als gevolg hiervan wordt op facturen nu een 0 weergegeven voor nummers die worden voorafgegaan door een tekenreeks. [ GitHub-34451 ](https://github.com/magento/magento2/issues/34451)

### Logboekregistratie

<!-- MC-42360-->

* Adobe Commerce maakt geen logboekvermeldingen meer voor mislukte API-aanroepen die bulkacties uitvoeren in **systeem- >** bulkacties ****. Eerder, werden de permanente ingangen voor ontbroken API vraag toegevoegd aan het bulkactielogboek.
...
* `curl_init()` retourneert nu een `CurlHandle` -object in plaats van een resource. De functie `curl_close()` heeft geen effect meer. In plaats daarvan wordt de `CurlHandle` instantie automatisch vernietigd als er niet meer naar wordt verwezen. [ GitHub-33781 ](https://github.com/magento/magento2/issues/33781)

<!-- AC-1397-->

* Fouten opgelost in MFTF-tests voor downloadbare producten. [ GitHub-34270 ](https://github.com/magento/magento2/issues/34270)

<!-- magento/magento2/pull/33530-->

* Verwijderd `CliCacheFlushActionGroup` uit `CatalogSearch`, `GroupedProduct`, `Newsletter`, `Paypal`, , `Quote`en `Review` modules. [ GitHub-33531 ](https://github.com/magento/magento2/issues/33531)

<!-- AC-515-->

* Niet-strikte vergelijkingen tussen getallen en niet-numerieke tekenreeksen werken nu door het getal naar de tekenreeks te werpen en de tekenreeksen te vergelijken. Vergelijkingen tussen getallen en numerieke tekenreeksen blijven werken zoals voorheen. Dit betekent dat 0 == &quot;not-a-number&quot; nu als onwaar wordt beschouwd. [ GitHub-33780 ](https://github.com/magento/magento2/issues/33780)
...
* Facturen voor bestellingen die worden betaald met winkeltegoed en waarop vaste productbelastingen (FPT) zijn toegepast, bevatten nu het juiste eindtotaal.

<!-- MC-42332-->

* De detailpagina van de beheerdersorder wordt nu geladen zoals verwacht. Voorheen gaf Adobe Commerce de volgende foutmelding bij het laden van de detailpagina van de bestelling voor bestellingen met bepaalde belastingen: `Call to a member function getId() on array`.

<!-- MC-38521-->

* Beheerders met beperkte machtigingen, waaronder bestel- en verzendprivileges, kunnen de bestelpagina nu bekijken zoals verwacht. Voorheen gaf Adobe Commerce een foutmelding wanneer een beheerder met deze machtigingen een bestelling probeerde te bekijken. [ GitHub-14633 ](https://github.com/magento/magento2/issues/14633)
...
* De juiste winkel-id wordt nu gebruikt om opgeslagen creditcards op te halen tijdens de plaatsing van de Admin-order. Opgeslagen creditcards worden nu weergegeven tijdens het plaatsen van de bestelling volgens de configuratie-instelling van het websitebereik. Wanneer een beheerder voorheen probeerde een order voor een klant te maken vanuit de beheerder en de methode Opgeslagen creditcards selecteerde, waren er geen opties beschikbaar voor opgeslagen kaarten.

<!-- AC-344-->

* Afwijkingen met de weergave van PayPal-tegoeden van cadeaubonbedragen zijn opgelost. Eerder, toen PayPal-krediet werd toegelaten en de veelvoudige geschenkkaartbedragen werden gevormd, als een verkoopster het bedrag voor de waarde van een geschenkkaart veranderde, werkte de storefront niet het bedrag voor afbetalingsbetalingen bij. Dit was een bekend probleem in Adobe Commerce 2.4.3.
...
* PayPal Payflow Pro voert nu vertraagde vastlegging uit met de juiste bovenliggende transactie-id&#39;s in implementaties waarbij de PayPal Payflow Pro-gateway met **betalingsactie** is ingesteld op **Autorisatie**. Een **verkooptransactie** wordt naar Payflow verzonden met een bovenliggende transactie-id van het kluistoken dat zoals verwacht bij het afrekenen is gemaakt. Voorheen werd een **Delayed Capture-transactie**  gemaakt en verzonden naar PayPal Payflow. [ GitHub-33445 ](https://github.com/magento/magento2/issues/33445)

<!-- MC-42442-->

* Shoppers worden nu zoals verwacht doorgestuurd naar de succespagina nadat de betaling met PayPal Payments is voltooid. Eerder werden kopers niet omgeleid naar deze pagina, ook al is de betaling verwerkt en zijn er geen fouten geregistreerd.

### Prestaties

<!-- AC-705-->

* Een index toegevoegd om `magento_giftcardaccount.code` de prestaties te verbeteren.

<!-- MC-42158-->

* De uitvoeringstijd van de modulelijst is verbeterd.

<!-- AC-932-->

* De prestaties van de cache voor populaire zoektermen zijn verbeterd. Voorheen gold dat hoe groter de `search_query` tabel, hoe langer het duurde om een zoekopdracht te voltooien. [ GitHub-27559 ](https://github.com/magento/magento2/issues/27559)

<!-- MC-42892-->

* Adobe Commerce past het formaat van afbeeldingen standaard synchroon aan tijdens het opslaan van het product. Winkeliers kunnen nu het formaat van afbeeldingen wijzigen als een asynchroon proces op de achtergrond door kleine bewerkingen in een `di.xml` bestand.
...
* Het saldo van beloningspunten en de geschiedenis van beloningspunten geven nu hetzelfde saldo weer. Verlopen punten worden nu correct afgetrokken. Voorheen gaf het saldo van beloningspunten het verlopen aantal punten niet nauwkeurig weer.

<!-- ACP2E-158-->

* Verlopen beloningspunten worden nu gemarkeerd als **Verlopen** in het gedeelte Reden van het raster Beloningspuntengeschiedenis van de winkel.
...
* De `Magento\GraphQl\CatalogGraphQl\ProductSearchTest` test geeft deze fout niet langer wanneer deze wordt uitgevoerd met AWS Elasticsearch: `Magento\GraphQl\CatalogGraphQl\ProductSearchTest::testSearchSuggestions Failed asserting that an array is not empty. /var/www/html/dev/tests/api-functional/testsuite/Magento/GraphQl/CatalogGraphQl/ProductSearchTest.php:94 /var/www/html/dev/tests/api-functional/framework/Magento/TestFramework/TestCase/GraphQlAbstract.php:257`.

<!-- magento/magento2/pull/33699-->

* Talloze niet-kritieke waarschuwingen voor eenheidstests opgelost door de naam van alle nepvariabelen te wijzigen om het `Mock` achtervoegsel op te nemen en verwijzingen naar een afgeschafte Object Manager-klasse te verwijderen. [ GitHub-33695 ](https://github.com/magento/magento2/issues/33695)

* De `Magento\Framework\MessageQueue\TopologyTest` test is nu compatibel met de AWS MQ voor [!DNL RabbitMQ] service. Eerder mislukte deze test met dit bericht: `Invalid exchange configuration: magento-topic-based-exchange1 Failed asserting that two arrays are equal`. <!-- AC-1248-->

<!-- magento/magento2/pull/34454-->

* Unittests zijn nu compatibel met PHP 8.1. [GitHub-34441](https://github.com/magento/magento2/issues/34441)
...
* De `UserExpiration` validator mislukt niet meer met `de_DE` en `uk_UA` locales. Voorheen gaf Adobe Commerce een foutmelding wanneer een beheerder probeerde een vervaldatum in te stellen bij het maken van een nieuwe gebruiker vanuit de beheerder met landinstellingen ingesteld op `de_DE` of `uk_UA`. [ GitHub-32497 ](https://github.com/magento/magento2/issues/32497)

<!-- MC-41583 AC-989-->

* Namen van Zwitserse regio&#39;s worden nu consequent in het Engels weergegeven in de formulieren voor het maken of bewerken van adressen. [ GitHub-32602 ](https://github.com/magento/magento2/issues/32602)
...
* Knoppen in de etalage hebben nu unieke, beschrijvende toegankelijke namen. Voorheen hadden gesplitste knoppen met een tekstknop en een aangrenzende pijl naar beneden dezelfde toegankelijke naam.

<!-- AC-1168-->

* Er is een **optie voor de weergave Vandaag** toegevoegd aan de dashboardgrafieken Admin-orders en -bedragen om een overzicht te geven van verkopen en bestellingen gedurende de dag. [ GitHub-34008 ](https://github.com/magento/magento2/issues/34008)

<!-- AC-1481-->

* De actieve (klik)status van de knop Sluiten **met meerdere selecties** werkt nu consistent. [ GitHub-34338 ](https://github.com/magento/magento2/issues/34338)
...
* Het toevoegen van een gemaskeerde citaat-ID aan `QuoteRepository::getList` veroorzaakt niet langer een oneindige lus. Voorheen werd dit gedrag veroorzaakt door het gebruik van de betalingsverwerker die op de REST API vertrouwde om bestellingen bij te werken. [ GitHub-33675 ](https://github.com/magento/magento2/issues/33675)

<!-- MC-41612-->

* De `/rest/all/V1/categories/<above_created_category_id>` PUT-aanroep respecteert nu de waarden die zijn gespecificeerd door `available_sort_by`.  Voorheen was de vervolgkeuzelijst Beschikbare productvermeldingen Sorteren op geselecteerd met alle mogelijke opties op de pagina Categorie en was het **selectievakje Alles** gebruiken ingeschakeld. [ GitHub-32596 ](https://github.com/magento/magento2/issues/32596)
...
* Er is een fout gecorrigeerd die optrad wanneer **Winkelcode toevoegen aan URL&#39;s** is ingesteld op **Ja** en een REST-aanroep geen winkelcode bevatte. Hoewel de payload geldig was, heeft de aanroep de volgende fout geretourneerd: `Fatal error: Allowed memory size of ********** bytes exhausted (tried to allocate **** bytes) in vendor/magento/module-store/Model/StoreRepository.php on line 75`.
...
**Probleem**: Winkeliers kunnen meldingen over downgrades van pakketversies opmerken tijdens de upgrade van Adobe Commerce 2.4.4 naar Adobe Commerce 2.4.4-p1. Deze berichten kunnen worden genegeerd. De discrepantie in pakketversies is het gevolg van afwijkingen tijdens het genereren van pakketten. Dit heeft geen invloed op de productfunctionaliteit. Zie de [ Pakketten die na bevordering van 2.4.4 aan 2.4.4-p1 ](https://support.magento.com/hc/en-us/articles/8214752983949) artikel van de Kennisbank voor een bespreking van beïnvloede scenario&#39;s en alternerende actie worden gedowngraded.

**Probleem**: winkeliers kunnen geen gedeeltelijke terugbetalingen indienen voor bestellingen die via Braintree met Apple Pay zijn betaald. Wanneer een winkelier een creditnota probeert te maken voor een gedeeltelijke terugbetaling van de orderfactuur, kan het **veld Aantal te retourneren** niet worden bewerkt.  **Tijdelijke oplossing**: Patch `braintree-disabled-partial-capture-for-applepay-googlepay.patch`toepassen . Zie het [Knowledge Base-artikel Adobe Commerce 2.4.4: Kan geen gedeeltelijke facturen](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) maken.  <!-- BUNDLE-3088-->

**Probleem**: shoppers kunnen geen product aan hun winkelwagentje toevoegen als er geen opties zijn geselecteerd in Admin **Store** >  **Configuration** > **General** > **Country Options** > **Allowcountries**. Adobe Commerce geeft in plaats daarvan de volgende consolefout weer: `Failed to load resource: the server responded with a status of 400 (Bad Request)`. <!-- AC-2564-->

**Probleem**: De beschrijving van Dotdigital op **Find Partners and Extensions** > **Magento Marketplace** is verouderd. Hier is een bijgewerkte beschrijving:

Dotdigital is een platform voor klantbetrokkenheid dat digitale marketeers en ontwikkelaars helpt bij het leveren van communicatie tijdens het hele klanttraject. Wij maken gebruik van de kracht van klantgegevens, die betrokkenheid, omzetting en loyaliteit voor merken aandrijven aangezien zij groeien en schrapen. Klanten houden van ons eenvoudig te gebruiken platform dat gegevens van eerste partijen over de systemen verbindt, krachtige inzichten en het automatiseren van voorspellende dwars-kanaalberichten op zich neemt.
...
## Toolkits voor migratie

De Data Migration Tool helpt bij het overzetten van bestaande Magento 1.x opslaggegevens naar Magento 2.x. Deze opdrachtregelinterface bevat verificatie-, voortgangs-, logboek- en testfuncties. Zie [De tool](../../../tools/data-migration-tool/install.md) voor gegevensmigratie installeren voor installatie-instructies. Overweeg om de [opslagplaats](https://github.com/magento/data-migration-tool) voor gegevensmigratie te verkennen of eraan bij te dragen.

De [Code Migration Toolkit](https://github.com/magento-commerce/code-migration) helpt bij het overzetten van bestaande Magento 1.x-winkelextensies en -aanpassingen naar Magento 2.x. De opdrachtregelinterface bevat scripts voor het converteren van Magento 1.x-modules en lay-outs.
